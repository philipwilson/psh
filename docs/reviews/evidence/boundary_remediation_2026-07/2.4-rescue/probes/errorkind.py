...
CASES = {
    ...
    # --- EXIT-TRAP TEARDOWN (round 3: the action's OWN text is invalid) ---
    # Round 2's corpus had only a VALID-action trap row, which is why the
    # teardown crash was invisible to it.
    "t1_teardown_bare":  (b"trap 'echo T; echo $(fi); echo T2' EXIT\necho B\n", {}),
    "t2_teardown_exit3": (b"trap 'echo T; echo $(fi)' EXIT\necho B\nexit 3\n", {}),
    "t3_teardown_sub":   (b"( trap 'echo CT; echo $(fi)' EXIT; echo IN )\necho AFTER rc=$?\n", {}),
    "t4_teardown_cmdsub": (b"x=$( trap 'echo $(fi)' EXIT; echo IN )\necho AFTER rc=$? x=$x\n", {}),
    "t5_teardown_ateof": (b"trap 'echo T; echo $(if); echo T2' EXIT\necho B\n", {}),
    "t6_teardown_eval":  (b'trap \'eval "echo \\$(fi)"\' EXIT\necho B\n', {}),
