...
| **psh loses EXIT-trap output on fatal-signal death (top-level shell)** | ~1/120 (out-of-pytest reproducer: subprocess+sentinel+os.kill, 120 runs/shell — psh 1/120 lost `cleanup\n`, bash 0/120, both rc −15); SIBLING FACE verified independently: trap's `exit 0` can win outright → psh exits 0 instead of dying by signal (1/100; bash 100/100 rc −15). CLI-REACHABLE (`psh script.sh` + `kill -TERM`). Forked-child path flushes (`child_policy.py#die_by_signal`); top-level signal path does not obviously share the discipline. Live sentinel test stays un-quarantined by ruling. | **Wave-1 RIDER slot 1.3b (v0.754.0)** — production fix sanctioned; oracle must count BOTH faces |
