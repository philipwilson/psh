...
|---|---|---|---|
| HIGH-1 oracle false-green + bypass | 1.1 + 1.2 | CONFIRMED: `yes` → both rc −9, 8 MB truncated, verdict IDENTICAL; ~30 direct-subprocess differential modules | `yes` discriminator → non-`Completed` harness failure; zero direct launches (AST guard + synthetic offender) |
