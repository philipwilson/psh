# Changelog history (pre-v0.200.0)

Older PSH release notes, split out of the top-level `CHANGELOG.md` on
2026-06-12 to keep that file focused on recent history. This file
contains every entry from the first release up to and including
v0.199.0; entries from v0.200.0 onward live in `CHANGELOG.md` at the
repository root. The format is identical:
`VERSION (DATE) - Title` followed by bullet points.
