...
2. **Tokenization**: `ModularLexer` + recognizer registry; `word_fusion.fuse_words` composites adjacent word pieces into one complete WORD token FIRST; `KeywordNormalizer` then classifies only complete unquoted literal words (fusion-first, v0.731 - a keyword prefix of a composite word is never promoted); `tokenize_with_heredocs` collects bodies
...
9. **Narrow Service Protocols (dependency direction)**: migrated boundaries depend on the narrow `psh/protocols` interfaces (`ExpansionRuntime`, `IOContext`, `JobRuntime`, `LocaleAccess`, `ExpansionHost`) rather than the whole `Shell`; the import edge is one-way — an implementation may import a protocol, a protocol never imports an implementation (campaign Q1; enforced by `tests/unit/tooling/test_protocol_layering_q1.py`, and the full-`Shell` consumer set — 8 full-`Shell` consumers across 4 modules as of v0.777.0 — only shrinks via `tests/unit/tooling/test_shell_consumer_ratchet_q1.py`)
...
- **`psh/lexer/token_parts.py`** - TokenPart class (per-part word metadata)
- **`psh/lexer/word_fusion.py`** - fuses adjacent word pieces into one WORD token
...
- Reserved words are recognized only as complete unquoted literal words in grammar positions; bare command-position `in` is a syntax error as in bash, while the for/case subject positions legitimately accept it (v0.731)
