...
| `rc_loader.py` | `load_rc_file` / `is_safe_rc_file` - startup RC loading. Called ONLY from `Shell.run_invocation_startup` (shell.py) — the explicit, idempotent startup step that runs AFTER the full invocation (options, parser, positionals) is applied and NEVER during `Shell.__init__` (campaign F1; pinned by tests/system/invocation/). The rc runs for every interactive-FAMILY shell, including `-ic` and `-i script.sh` (bash). The rc EXECUTES through the ONE sourced-program service (`psh/scripting/program_source.py#execute_sourced_file` — the same service `source`/`.` uses; campaign F3): it has source depth (`return 7` ends the rc cleanly with no diagnostic, its status discarded, startup continues — bash), never fires the RETURN trap, never bang-expands its lines, and applies the string-read NUL policy with no binary limit. Pinned by tests/system/source_service/. |
