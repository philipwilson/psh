"""Pure char-level helper functions for lexer operations.

Stateless, pure functions used by the lexer without coupling to its internal
state: quote/escape tracking (QuoteState), delimiter matching, escape-sequence
decoding, and variable-name extraction. Easier to test, reuse, and reason
about in isolation.

Two former residents grew into modules of their own:

* the grammar-aware ``$(...)`` extent scanner lives in ``cmdsub_scanner.py``
  (a parser component, not a char-level helper);
* the word-shape scanners (glob brackets, assignment prefixes, inline
...
def scan_double_paren_arithmetic(
    ...
    while pos < n:
        ...
        # (Call-time import: cmdsub_scanner imports this module at top
        # level, so the dependency back onto the scanner must stay lazy.)
...
def validate_brace_expansion(
    ...
    while pos < n:
        ...
        if input_text.startswith('$(', pos):
            ...
            # (Call-time import — see find_balanced_double_parentheses.)
            from .cmdsub_scanner import find_command_substitution_end
            end, found = find_command_substitution_end(input_text, pos + 2)
            if not found:
                break
