    ...
    def expand(self, tokens):
        ...
        for i, tok in enumerate(tokens):
            ...
            if (tok.type == TokenType.WORD and tok.quote_type is None
                ...
                # Composite = glued to a word-forming neighbour (e.g. {a,b}"x"
                # or "x"{a,b}). Adjacency to a non-word token (an operator like
                # '(' in arr=({a,b} c), or EOF) does not make it composite.
                glued_prev = (
                    getattr(tok, 'adjacent_to_previous', False)
                    and prev_tok is not None and prev_tok.type in word_like
                )
                glued_next = (
                    next_tok is not None and next_tok.type in word_like
                    and getattr(next_tok, 'adjacent_to_previous', False)
                )
                is_composite = glued_prev or glued_next
                is_assignment = zone_active and bool(_ASSIGNMENT_RE.match(tok.value))

                if not is_composite and not is_assignment:
                    try:
                    ...
                    if not (len(expansions) == 1 and expansions[0] == tok.value):
                        ...
                        # ({a,,b} -> a b, {,b} -> b), matching what the old
                        # expand-then-re-lex path did via word splitting. An
                        # empty item fused with a prefix/postfix is non-empty
                        # and survives (a{,b} -> a ab).
                        expansions = [e for e in expansions if e != '']
                        produced = self._make_word_tokens(tok, expansions)

            out.extend(produced)
            zone_active = self._next_zone(tok, zone_active, reset_types)

        return out

    @staticmethod
    def _next_zone(tok, zone_active, reset_types):
        from .token_types import TokenType
        if tok.type in reset_types:
            return True
        if tok.type == TokenType.WORD:
            # An assignment word keeps the prefix open (a=1 b=2 cmd); any other
            # word is the command name (or an argument) and ends the prefix.
            if zone_active and tok.value and _ASSIGNMENT_RE.match(tok.value):
                return True
            return False
        return zone_active  # operators / redirections: leave the zone as-is
