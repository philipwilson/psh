...
| `command.py` | `CommandExecutor` - simple command dispatch (expansion, one resolution, redirections). Normalizes the command word, builds the overlay, EXPANDS the prefix values (`expand_prefix`), then resolves ONCE (`resolve_command`) against that authoritative state and routes the expanded bindings (`commit_prefix`) — so a side effect inside a prefix VALUE reaches the dispatch decision it governs (slot 3.4). Drives the scope model / `exec` shortcut / POSIX prefix-error branch / persistence from the returned `ResolvedCommand` (never a raw-name recompute — #20 H10) |
...
rc=$?'` prints `rc=5`. Which fd a redirection targets is `io_redirect/redirect_program.py#target_fd_of`,
