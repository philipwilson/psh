...
# Shared job vocabulary lives in core so the job builtins can name it without
# importing the executor (breaks the builtins<->executor cycle, P4). Imported
# here so JobManager's own code — and callers reaching for
# ``executor.job_control.JobState`` — keep working unchanged.
from ..core.job_state import (  # noqa: F401  (re-exported for existing callers)
    JobSpecOutcome,
    JobSpecResult,
    JobState,
...
class Job:
    ...
    def format_status(self, is_current: bool, is_previous: bool,
        ...
        if pid is not None:
            return f"[{self.job_id}]{marker} {pid} {state_str:<27}{self.command}{suffix}"
        return f"[{self.job_id}]{marker}  {state_str:<27}{self.command}{suffix}"
...
class JobManager:
    ...
    def _print_completion_notice(self, job: 'Job') -> None:
        ...
        # Same 27-column status field as the `jobs` listing (bash 5.3.15,
        # PTY-probed: `[1]+  Done` + 23 spaces + command).
        print(f"[{job.job_id}]{marker}  {label:<27}{job.command}",
