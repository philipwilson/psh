...
class SpecialBuiltinUsageError(PshError):
    """Typed outcome: a POSIX special builtin hit a USAGE or OPERAND error
    bash treats as fatal in posix mode.
    ...
    ``readonly``/``export``, a syntax error inside ``eval``/a sourced file
    (raised at the nested buffered-command boundary), and — since bash 5.3
    — the OPERAND errors: an invalid identifier given to
    ``export``/``readonly``, and ``unset`` refusing a readonly variable,
    array, array element or function. ``status`` carries the builtin's own
    exit status (2 for option/syntax usage errors, 1 for the readonly,
    identifier and dot cases).
    ...
    pre-policy behavior. Errors that still do NOT raise this: a bad signal
    spec to ``trap``, ``unset 1bad`` (a silent rc-0 no-op in bash 5.3),
    ``unset`` of a non-array subscript, and every ``declare``/``typeset``
    diagnostic (declare is not a special builtin). Truth table:
    docs/reviews/posix_special_builtin_exit_matrix_2026-07-07.md.

    ``suppressible`` records bash's TWO exit classes (probe-verified vs
    bash 5.3.15 in all three input modes): INVALID-OPTION usage errors, a
    top-level ``return`` and the 5.3 operand class are SUPPRESSED in
    errexit-exempt contexts (if/while/until conditions, non-final &&/||
    members, ``!``-negated pipelines — reaching through functions, brace
    groups, subshells and, on 5.3, through an ``eval``/``.`` boundary):
    the builtin merely fails with ``status`` there (``set -o posix; eval
    'set -q' || echo caught`` prints ``caught``, rc 0). A TRAP ACTION is
    the one boundary the suppression does not cross. The eval/dot SYNTAX
    errors, missing/unreadable dot-file and readonly-ASSIGNMENT outcomes
    are HARD (``suppressible=False``): bash exits even inside a guard
