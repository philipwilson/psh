...
def run_psh_c(script):
    r = run_psh(["-c", script], timeout=30)
    assert is_comparable(r), r
    return r.returncode, r.stdout, r.stderr


def run_bash_c(script):
    r = run_bash(["-c", script], timeout=30)
    assert is_comparable(r), r
    return r.returncode, r.stdout, r.stderr
...
def test_set_listing_matches_bash(tag):
    ...
    _, pout, _ = run_psh_c(script)
    _, bout, _ = run_bash_c(script)
