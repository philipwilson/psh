...
CORPUS = [
    ...
    # --- EMPTY parts inside the same run. `""`, `''`, `$''` and `$""` each
    #     parse to a zero-length literal, so they separate nothing in the
    #     EMITTED text: the formatter merges the whole same-quote run and
    #     display_text drops the quotes. The spelling therefore has to look
    #     past them, not at the syntactically next part ---
    ("dq_empty_adjacent", 'echo "$v"""' + '"x"'),
    ("dq_empty_digit", 'echo "$v"""' + '"9"'),
    ("dq_empty_underscore", 'echo "$v"""' + '"_z"'),
    ("dq_empty_double", 'echo "$v"""' + '""' + '"x"'),
    ("dq_empty_prefix", 'echo "a$v"""' + '"x"'),
    ("dq_empty_locale", 'echo "$v"$"""x"'),
    ("dq_empty_sq_between", 'echo "$v"' + "''" + '"x"'),
    ("dq_empty_ansic_between", 'echo "$v"$' + "''" + '"x"'),
    ("dq_empty_then_bare", 'echo "$v"""x'),
    ("bare_empty_dq_between", 'echo $v""x'),
    ("bare_empty_sq_between", "echo $v''x"),
    ("dq_empty_between_expansions", 'echo "$v"""' + '"$y"'),
    ("dq_empty_assign", 'z2="$v"""' + '"x"; echo "$z2"'),
    ("dq_empty_local", 'local w3="$v"""' + '"x"; echo "$w3"'),
    ("dq_empty_for", 'for k2 in "$v"""' + '"x"; do echo "[$k2]"; done'),
    ("dq_empty_case",
     'case "$v"""' + '"x" in 1x) echo m;; *) echo n;; esac'),
    ("dq_empty_dbl_bracket",
     '[[ "$v"""' + '"x" == 1x ]] && echo yes || echo no'),
    ("dq_empty_array", 'a4=("$v"""' + '"x"); echo "${a4[@]}"'),
    ("dq_empty_herestring", 'cat <<< "$v"""' + '"x"'),
    ("dq_empty_redirect",
     'echo hi > "$v"""' + '"x"; cat 1x; rm -f 1x'),
    ("dq_empty_nested_func",
     'g2() { echo "$v"""' + '"x"; }; g2'),
    ("dq_empty_assoc_key",
     'declare -A m2; m2["$v"""' + '"x"]=Z; echo "${!m2[@]}"'),
    # controls: an empty part that changes nothing must change nothing
    ("dq_empty_braced_control", 'echo "${v}"""' + '"x"'),
    ("dq_empty_special_control", 'echo "$#"""' + '"x"'),
    ("dq_empty_first_control", 'echo """$v""x"'),
