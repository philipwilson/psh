...
- **`read -N count` now honors `-t timeout`** (the A5 rider): previously `-t` was ignored entirely — `read -t 1 -N 3` blocked until EOF, or forever without one; the tty arm hung too (proven by PTY pins after a wrong-tree probe was self-caught and hardened). Now both arms share the `-n` path's monotonic deadline with bash's outcome mapping: full count → 0, EOF short → 1, timeout → 142 with the partial input assigned. Two rider cases promoted to the golden corpus (compare-bash 3,042 → 3,046, a declared count increase with zero flips).
- **Three divergences declared and registered, none absorbed**: bash assigns stranded partial-multibyte bytes at a `-t` timeout while psh holds them for the next read (deferred to slot 4B.4 with divergence pins that flip loudly if ruled); bash counts post-escape characters for `-N` while psh counts raw (reaches the exit status); `read -s -N` at a TTY echoes where bash suppresses (pre-existing, report-only).
- Carry #21 (`read -N` mixed valid+malformed hybrid) re-ruled RE-CARRY on a fresh 24-cell probe — the hybrid is psh's own documented model — with its first-ever test pin and a proven-empty base-vs-tip behavior diff.
- 92 new test nodes (including 3 PTY cells and 7 mutation locks certified on a fresh checkout); no measurable performance cost expected; not measured. Remediation campaign: MEDIUM-2 CLOSED (evidence in `docs/reviews/evidence/boundary_remediation_2026-07/4b.2-rescue/`).

## 0.770.0 (2026-08-07) - Immutable variable reads (remediation slot 4B.1, MEDIUM-5)

- **The variable-read result is now immutable**: `VariableLookup` exposes `status`/`value`/`is_set`/`is_present` as read-only properties over private `__slots__` — plain assignment or deletion on any public name raises on fresh results and on the shared singletons alike. At base, mutating the shared `_MISSING` singleton poisoned every future miss process-wide (`${unset+word}` began firing for unrelated names), and `lookup(name).binding.value = ...` silently bypassed readonly enforcement, nameref write guards, and the exported-environment observer. All three routes are pinned red-on-base.
- **The `binding` field is gone** (ruled OMIT): a double-derived census found ZERO production readers — the documented consumers (`${x@a}`, `declare -p`) actually use the write engine's own `get_variable_object` family. With no field there is no path from a lookup result to a live variable cell at all, so the readonly/nameref/observer/export coherence guarantee holds structurally rather than defensively. Cell-level assertions in tests now go through the sanctioned surfaces.
- **Both nullary results are shared frozen singletons**: `MISSING` (as before, now immutable) and `PRESENT_UNSET` (new — it carried no data once `binding` left, so it stopped allocating; measured slightly FASTER than base). Lookup equality is status+value; results remain unhashable by decision.
- **The documented non-frozen perf ruling was overturned by measurement, not argument**: its premise — "`lookup()` sits on the shell's hottest read path" — is false (the hot string path builds no lookup; `lookup()` has ONE production caller, the `${x+w}`/`${x-w}` set-ness test, and constructs zero objects on realistic workloads and at startup), and the chosen representation costs 1.000x anyway, so immutability required none of the feared 3x construction tax the old docstring cited.
- **No shell-observable change**: compare-bash 3,042/26 EXACT and conformance counts unchanged. 58 new pin cells (mutation attempts per field × surface, poisoning end-to-end, authority-coherence matrix, composition cells, one labelled control documenting the declared open-class circumvention boundary); 5 mutation locks with cross-run-identical kill counts.
- Remediation campaign: MEDIUM-5 CLOSED (evidence in `docs/reviews/evidence/boundary_remediation_2026-07/4b.1-rescue/`); campaign-wide rule adopted mid-slot: every exit-status-bearing check (ruff/mypy/pytest) runs unpiped or branches on its own exit code.
...
- Carry register grown to 35 rows, each both-sides characterization-pinned - including the sticky-hash of failed non-executable candidates (bash implicitly hashes them; corrected a false design-claim record), `[[` unquoted-operand quote-provenance (full fix = a provenance-carrying arithmetic input contract, registered for the future campaign), and three piped-interactive-mode history faces.
- 131 new pins across the slot; gate 20,315/0; compare-bash byte-exact; mypy 274 files.

## 0.749.0 (2026-07-20) - Boundary campaign Q3: documentation closure and the committed closing report
- **Campaign closing report committed** (Boundary Integrity Campaign, package Q3 - the final dev slot): `docs/reviews/boundary_campaign_close_2026-07.md` carries the final boundary ledger (25 packages, v0.725.0-v0.749.0), the verified producer/consumer census of all 27 canonical representations, the deliberate-loss/documented-divergence registry assembled from the slot ledgers, the 17-item post-campaign carry register, and the H1-H19+C1 closure table (every pin verified present and passing; H15 and H19 PARTIAL with carried residuals, all others closed or characterized).
- **Documentation closure** (exit criterion 15.9): six stale architecture/tour claims fixed - the worst described the W3-deleted glob-to-regex matching path as the current engine and a tokenizer pass retired in v0.678 as live; ARCHITECTURE.md now names `psh/protocols` and the one-way dependency direction (Invariant #9).
- **Compatibility-surface sweep**: the `InputReader` -> `InputCursor` back-compat alias (psh/builtins/input_reader.py) is **REMOVED** - a public-symbol removal per the Q3 deletion doctrine; it had zero production consumers and its two test consumers were migrated. Remaining adapters are documented with owner and reason (the `CommandAccumulator` Option-A terminal adapter is sanctioned permanent). The four Q2-retained pattern-conversion helpers are documented as PERMANENT reference oracles (integrator ruling) - no longer deferred deletions.
- The F9/Q1 git-range self-checks now warn loudly (naming the lost protection) before skipping on gitless checkouts. +2 structural tests; gate 20,190/0; mypy 274 files; zero behavior change.

## 0.748.0 (2026-07-20) - Boundary campaign Q2: cross-cutting ratchets
- **Nine tree-wide ratchet families, landed sequentially** (Boundary Integrity Campaign, package Q2): option walkers vs justified hand-written parsers; `getattr`/`hasattr` on declared fields; broad `ValueError`/`TypeError` catches as control flow; command-registry reads outside resolution (R3 widened); redirect target/flag re-derivation incl. C1 procsub-sniff tree-wide (R1 extended); Bash-oracle bypasses (E2 extended); syntax-bearing raw AST fields drift-lock; visitor recursion outside `walk_ast` (S5 extended); incomplete public signatures in migrated packages (mypy disallow flags, git-self-checked module set). Every allowlist is enumerated from a recorded command, justified per entry, shrink-only, and tested with a synthetic offender run red - detectors match bare, quoted, tuple-constant, and aliased shapes, with declared-out-of-scope lines for the rest.
- **`_offset_line_numbers` migrated to `walk_ast`** - the S5 sanctioned exception is closed; `$LINENO` behavior bash-exact.
- **Doctrine-checked dead deletions**: `glob_to_regex_body` and `InputCursorRegistry.reset` deleted (zero live consumers verified across exports/docs/reflection/dispatch); four census candidates RETAINED as live differential test oracles (recorded, not silently kept). One unreachable-defensive `hasattr` guard deleted after its reachability claim was disproven.
- Doc-pointer and census guards widened beyond `psh/` (root and tests CLAUDE.md pointers, tools/ corpus). 61 net new structural tests; gate 20,188/0; compare-bash byte-exact; mypy 274 files.

## 0.747.0 (2026-07-20) - Boundary campaign Q1: narrow runtime protocols
- **Five service protocols** (Boundary Integrity Campaign, package Q1): `psh/protocols` defines `VariableAccess`, `ExpansionContext`, `IOContext`, `JobRuntime`, and `LocaleContext` as typing Protocols against their canonical producers, with runtime conformance pins and member-set freeze pins. Protocol modules import typing/stdlib only - an import-layering guard enforces the direction (implementations may import protocols, never the reverse).
- **Migrations**: `make_reader` and `InputCursorRegistry.cursor_for_fd` now accept `IOContext` (they read only `.stdin`); `ForegroundJobSession` accepts `JobRuntime` instead of concrete `JobManager` (mypy-checked). Annotation-only - the shell satisfies every protocol structurally; zero behavior change (compare-bash byte-exact).
- **Constructor/import ratchet**: an enumeration-derived census of all 16 campaign-created modules (self-checked against git history by a live test) plus campaign-touched pre-existing files; the 6 remaining full-`Shell` consumers are frozen in a shrink-only allowlist, each with a written justification. The detector catches bare, quoted, wrapped (`Optional['Shell']`, `Callable[['Shell'], X]`), and unannotated `shell`-named parameters, with red self-tests.
- `VariableAccess`/`ExpansionContext`/`LocaleContext` ship as defined-and-frozen surface; consumer adoption is post-campaign. 22 structural tests; gate 20,127/0; mypy 274 files.

## 0.746.0 (2026-07-20) - Boundary campaign J1: one job lifecycle (J-phase complete)
- **One async signal policy, applied to every member** (Boundary Integrity Campaign, package J1, closes #20 H11): `AsyncJobPolicy(ignore_int_quit, redirect_stdin_from_devnull)` is computed once per launch from background status AND monitor mode (`set -m` now consulted - fixes a base divergence where a monitor-mode background single ignored SIGINT) and applied to every leaf member of a background job. A background pipeline member no longer dies 130/131 on delayed INT/QUIT where bash completes rc 0; `/dev/null` stdin stays SINGLE-only (bash leaves an async pipeline leader on the real stdin - probe-pinned).
- **One foreground-job transaction** (closes #20 H12): `ForegroundJobSession` owns registration, process-group/terminal transfer, waiting, signal-death reporting, current-job rotation, and exception cleanup for external commands, pipelines, AND foreground subshells. Subshell signal deaths now report (`Terminated: 15` etc.) like bash; a pipeline-member subshell propagates its body's fatal signal via `child_policy.die_by_signal` so only the status-determining member announces (pipefail included). The `fg` resume path routes signal deaths to the same chokepoint (bash announced; psh was silent).
- **Typed hangup + reap ownership** (#20 H19, partially closed): typed `Job.no_hup` (`disown -h`), new `huponexit` option, and reap-registry/job-table separation, all honored on the ONE `Shell.shutdown` path. An interactive shell RECEIVING untrapped SIGHUP fans SIGHUP out to its jobs (SIGCONT first for stopped, `no_hup` honored), runs the EXIT trap, saves history, and dies 128+HUP - bash's `hangup_all_jobs`, verified with a tmux real-terminal construction after the python-pty probe family proved artifact-prone (all three parties' round-1 probes were construction-blind; recorded as a campaign lesson). A real terminal disconnect lands on the same path. Prompt-time reaping of detached children while the shell lives is a declared carry.
- **`wait` tests the returned pid separately from status**: `wait <pid>` on a disowned child now returns 0 like bash regardless of the child's exit status (psh previously reported "not a child of this shell"/127 for an exited-0 child and leaked the raw status otherwise).
- 47 new default-gate pins (6+ red-on-base) + 5 PTY pins promoted to the default gate; drift-lock sole-owner guard for the session; F2 signal-disposition ratchet extended cleanly (`die_by_signal` lives with the policy owner). Gate 20,105/0; mypy 273 files.

## 0.745.0 (2026-07-20) - Boundary campaign I4: typed history expansion (I-phase complete)
- **History expansion returns a typed result** (Boundary Integrity Campaign, package I4, completes the I-phase): the quote-aware scanner/resolver returns `HistoryExpansionResult`; recording and diagnostics consume `result.kind` - the forbidden reference-regex and every `expanded_text != original_text` inference are DELETED. Failed references, `:p` print-only, identical-expansion, and normal expansion are distinct outcomes, each pinned as its full (executed?, recorded-as?, rc, stderr) tuple against live bash.
- **Nine bash divergences fixed**: `:p` prints once to STDERR and records its expansion (bash-faithful, confirmed by the verifier's independent PTY re-derivation - `history -p` the builtin stays stdout and is newly WIRED, was a stub); `set +H` lines record verbatim; `!0` errors; `!#`, `:q`, `:x` added; heredoc BODIES never history-expand - including heredocs opened inside DOUBLE-QUOTED command substitutions, a base bug the verification bounce caught being claimed-but-not-covered (fixed with a local context-flags scanner that leaves S2's completeness oracle untouched; corpus byte-stable). Activation consumes F1's interactive-family + histexpand, never is_script_mode.
- **History files round-trip bytes** via UTF-8 + surrogateescape on all five paths (startup, save, append, read-new, rewrite - the I1 doctrine). The user-guide compatibility table's self-contradicting rows (claiming -p/:q/:x/!# unsupported) were corrected with conformance backing in the CLAIM_TESTS-mapped file.
- Verification bounce round (13th; 35/35 campaign blockers real, 0 false): the conditional `:p`-stream sanction was SATISFIED by PTY evidence; the dquoted-cmdsub heredoc composition and the user-guide contradiction were caught and fixed. 69 pins net (+12 bounce rows red-on-pre-fix); two new carry entries (plain-expansion echo stream; history -p failed-arg wording). Gate 20,054 expected; compare-bash 2,986 EXACT; conformance 2,539; mypy 272; PTY tier 105/2xf.

## 0.744.0 (2026-07-19) - Boundary campaign I3: resumable completeness and parsing
- **One completeness engine** (Boundary Integrity Campaign, package I3 [BOLD]): `ParserDriver.start_session()` -> `ParseSession` now owns multiline accumulation - the heredoc queue, line buffer, line-continuation detection (v0.674 mode-sensitivity relocated verbatim), trial parse -> typed `Complete | Incomplete | Invalid`, and deterministic operation counters. `CommandAccumulator` is a thin scripting adapter (history-preprocess injected as a callable, so the parser package stays scripting-free); every consumer is unchanged and the PS2/continuation behavior is byte-identical.
- **The S4 combinator outcome divergence is CLOSED** (the I3-readiness carry): a shared `detect_unclosed_expansion` producer feeds both parsers, so unclosed `$(`/`${`/`$((`/backtick/`<(` classify `Incomplete` in the combinator exactly as in RD (was `Invalid`); the disclosed-divergence pin flipped to a parity pin, red-on-base archived, RD proven byte-identical over a 24-input differential.
- **#20 H15 partially closed, honestly**: a dev spike proved bash reports mid-construct syntax errors IMMEDIATELY (PTY-proven; now locked by parity pins that run in the default gate), which forces per-feed parsing - so single-open-construct accumulation stays O(k^2) by oracle-derived necessity, pinned by a doubling-ratio characterization test carrying the full rationale. The genuinely linear families ARE linear and pinned by operation counts: heredoc bodies O(1)/line, multi-command streams O(N) via reset-per-command. Full closure requires a resumable-lexer/parser campaign, recorded on the post-campaign register.
- Verified by a 4-agent adversarial pass: PASS, ZERO blockers (eighth clean pass; 33/33 campaign blockers real, 0 false). 55 pins + 5 PTY parity rows (now run-by-default); integrator fixups swept stale pointers and promoted the PTY pins into the gate. A new pre-existing divergence found en route (trailing redirect at EOF in an open construct: psh Incomplete, bash immediate error) is probed and carried. Gate 19,983 expected; compare-bash 2,986 EXACT; conformance 2,504 EXACT; mypy 271.

## 0.743.0 (2026-07-19) - Boundary campaign I2: lazy ProgramSource for scripts
- **Script files and FIFOs are consumed on demand** (Boundary Integrity Campaign, package I2 [BOLD]): dual-mode `LazyFileInput` chosen by seekability - regular files block-buffer over a PRIVATE descriptor (over-read invisible; bounded <=64KB buffer), while pipes/FIFOs/`/dev/stdin` never-over-read through I1's shared `InputCursor` (a `/dev/stdin` script shares fd 0's open description - block-buffering starved in-script `read`, a divergence the probe matrix caught mid-design). Memory for a 19 MB script drops 118.7 -> 36.3 MB, bounded independent of file size; startup/execution unchanged.
- **bash's self-modifying-script semantics land**: bytes already consumed are never re-read; bytes appended past the read frontier ARE seen and executed (verified byte-exact at 19 pad sizes; block-size specifics documented as deliberate loss). The producer-waits-for-first-side-effect FIFO case passes. The script descriptor relocates to bash's fd-255 convention, CLOEXEC, owned by the source/activation lifecycle: `/dev/fd` now shows bash parity, user `exec 255>f` triggers relocation instead of clobbering, `{v}` named-fd allocation stays at 10. `source`/rc remain eager - probed: bash's source builtin is also eager (a self-appending sourced file does NOT see appended lines).
- All four I1 handoffs dispositioned: the deprecated `InputReader` alias has zero production consumers (StdinInput now builds its cursor through one chokepoint); the dual-cursor-on-stdin split is documented as designed (both cursors never-over-read, composing via the shared kernel offset - probe-verified); the close-drops and fresh-child-registry pins landed.
- Verified by a 4-agent adversarial pass: PASS, ZERO blockers (seventh clean pass; 33/33 campaign blockers real, 0 false) - the verifier independently re-ran every suite and battery at the final SHA (all exact) and extended the self-modify family (truncate/mv/unlink) and pad-size sweep. 35 pins (12 red-on-base); integrator fixup retired stale pre-I2 eager-script prose. Gate 19,923 expected; compare-bash 2,986 EXACT; conformance 2,504 EXACT; mypy 269.

## 0.742.0 (2026-07-19) - Boundary campaign I1: source-lifetime byte cursor
- **Input decoding is one surrogateescape byte cursor** (Boundary Integrity Campaign, package I1 [BOLD], opens the I-phase; closes #20 H16): the hand-rolled `errors='replace'` decode - which turned malformed bytes into U+FFFD and could eat record delimiters after a malformed lead - is DELETED. The renamed `InputCursor` decodes records through one incremental UTF-8 `surrogateescape` decoder, so malformed bytes round-trip byte-identically from `read` through variables to output, matching C-locale bash (byte matrix 23 -> 27 of 29 MATCH; the two residuals are documented deliberate divergences from UTF-8-locale libc `mbrtowc` quirks, pinned against `LC_ALL=C` bash).
- **Cursors are keyed by owned open-file-description identity** (`OpenDescription` + a per-shell `InputCursorRegistry`, SCOPED per ruling): same-fd `read -N` decoder carryover persists across builtin calls; `exec 0<file` rebinding creates a fresh cursor; the never-over-read discipline (record reads on pipes, kernel-offset sharing on dups) remains the primary contract - probing confirmed bash itself carries NO cross-call decoder state, so dup-alias and temp-frame carryover entries are parity-via-different-mechanism, recorded in a four-row deliberate-loss registry. The brief's replaying-fd-view conditional was answered NO with probe evidence. A false brief premise was corrected: R1 shipped no identity type (here-input sharing is behavioral); I1 creates the type for R1's later adoption.
- Verified by a 4-agent adversarial pass: PASS, ZERO blockers (sixth clean pass; 33/33 campaign blockers real, 0 false) - including the sharp export-to-child-env check (surrogate-escaped bytes reach children as raw bytes, no crash). Read path FASTER than base (while-read -7.8%, mapfile -7.6% via an invariant-guarded ASCII fast path). 35 pins (8 red-on-base); campaign ratchets caught and fixed an oracle-seam and an import-layering violation in the new code. Gate 19,888 expected; compare-bash 2,986 EXACT; conformance 2,504 EXACT; mypy 269.

## 0.741.0 (2026-07-19) - Boundary campaign R3: normalize, expand assignments, resolve once (R-phase complete)
- **Commands resolve ONCE into a typed decision** (Boundary Integrity Campaign, package R3; closes #20 H10 and completes the R-phase): `NormalizedCommandName` -> immutable `CommandEnvOverlay` (prefix-assignment FACTS: names, PATH-override, POSIX-override) -> `resolve_command` (the sole function/builtin dispatch reader) -> `ResolvedCommand` carrying dispatch target, precedence, POSIX-special status, assignment persistence, execution strategy, and exec/permanent-redirect policy. Scope creation and pop/promote are driven by the resolution - the three raw-name recompute sites are DELETED, and an AST ratchet (proven to bite those exact base sites) rejects registry reads outside the resolver.
- **H10 family fixed**: POSIX-special builtins shadowed by same-named functions now persist prefix assignments as bash does; posix-mode `exec` shadowed by a function reaches the exec builtin. Empty/unset-PATH command-not-found now says "No such file or directory" (rc 127) matching bash.
- **Verification bounce round** (12th; 33/33 campaign blockers real, 0 false) caught a subtle regression at the intersection of two campaigns: resolving before prefix application read the posix option before a `POSIXLY_CORRECT=1` prefix installed it (the v0.676 coupling) - masked by a subshell-constructed pin. Fixed with the bounce-derived rule: prefix posix-override detection is NAME-level, nameref write-through counts, readonly POSIXLY_CORRECT blocks the flip (the overlay performs the installer's readonly walk, predicting the outcome exactly). The brief's `effective_path` was deleted per ruling (values expand after resolution - pre-expanding only PATH would reorder side effects); lost perf-mitigation hunks (a checkout wipe during base benchmarking) were forensically traced and wired for real.
- 83 pins (red-on-base/tip per family); resolution overhead +0.8-1.8% on a re-lex-dominated synthetic (EMPTY_OVERLAY singleton + hoisted strategies tuple). Gate 19,853 expected; compare-bash 2,986 EXACT; conformance 2,504; mypy 268. The long-traveling empty-arith-subscript carry is dispositioned as a post-campaign item with a both-sides pin.

## 0.740.0 (2026-07-19) - Boundary campaign R1: ordered RedirectProgram
- **Redirects are a typed, source-ordered program** (Boundary Integrity Campaign, package R1 [BOLD]; closes #20 H4 + H8): `RedirectPlanner.plan_program` is the sole producer of `RedirectProgram` operations (`OpenFile`/`DupFd`/`CloseFd`/`HereInput` with typed targets, flags, locations); `apply_in_order` is the one semantic applicator all four dispatch sites walk (builtin / child / temporary-window / exec) - a deferred operation is unrepresentable, and the fd-vs-Python-stream difference is confined to each site's mechanical adapter.
- **H4 killed**: the builtin path's deferred-fd-close bookkeeping is DELETED - `echo hi 3>&- 4>&3` (fd 3 open) now errors exactly as bash instead of silently succeeding. **H8 killed**: builtin heredoc/here-string consumers share ONE open file description with children (`eval 'read x; cat' <<EOF` no longer replays); the temp-file-vs-pipe substrate difference vs bash's small-heredoc pipe is a documented deliberate loss with a deterministic lseek discriminator pin (bash's own file-backed regime matches psh exactly). Redirect diagnostics carry the `line N:` prefix per the errprefix convention; the dynamic-dup bad-fd diagnostic names the TARGET fd as bash does.
- **Verification bounce round** (11th; 28/28 campaign blockers real, 0 false) caught (a) a live shell-destroying regression no existing test could see - a builtin close+stdin-redirect freed fd 1 for reuse and the restore sequence closed it (fixed with F_DUPFD_CLOEXEC>=3 relocation of input-side dups + close-opened-streams-BEFORE-restore ordering; pinned with shell-still-alive assertions) - and (b) psh's missing `n>&n` leniency un-masked by the immediacy fix (bash treats self-dup as success even on a closed fd; implemented via one `is_self_dup` predicate across all universes, converging a pre-existing divergence). C1 structural-origin guard: expanded redirect text can never be reclassified as process-substitution syntax (guard + offender).
- 74 pins total (30 red-on-base/tip across the two rounds); gate 19,770 expected; compare-bash 2,986 EXACT; conformance 2,458 EXACT; mypy 267; perf noise-level on both redirect-heavy and plain paths.

## 0.739.0 (2026-07-19) - Boundary campaign R2: variable truth and environment materialization
- **Variable lookup is a typed tri-state** (Boundary Integrity Campaign, package R2 [BOLD], opens the R-phase; closes #20 H13): `ScopeManager.lookup()` returns `VariableLookup(MISSING | PRESENT_UNSET | VALUE, binding)` - a declared-unset local STOPS lookup instead of falling through, all seven `${x-...}`-family operators discriminate correctly, and the lookup-time environment fallback is DELETED (guard-locked: lookup never consults `state.env`; startup env imports atomically into exported cells; child environments are pure projections of visible exported cells).
- **bash's tempvar provenance model, derived from a 15-row discriminating matrix**: the `x=5 f` prefix binding carries provenance that a value-less `local x` inherits (value + export) at ANY visible call depth, while provenance-less instances and tombstones block - the depth-blind first cut was caught by verification. Dynamic specials are lexically scoped at seven sites through one predicate (`local RANDOM=5` masks the generator; `RANDOM=5 f` masks inside f; the drift-lock guard caught the seventh site - exported local SECONDS now materializes to children); bash refuses `local SECONDS` when the special is readonly; `unset -n` on a non-nameref is a silent no-op; `$!` absence is real absence.
- **Nameref attribute semantics completed**: attribute changes resolve nameref chains (except when modifying the nameref attribute itself), a cycle WARNS TWICE AND CONTINUES rc 0 as bash does, and - closing the standard upvar idiom - an attribute change through a nameref to a MISSING target creates the declared-unset attribute-carrying cell (`declare -n ref=$1; declare -i ref` now works; declare/readonly create in the local scope, export survives the function, exactly as probed).
- **Verification bounce round** (10th; 26/26 campaign blockers real, 0 false - the richest haul: a depth regression, a cycle-fatality divergence, a mutation-proven pin gap, a readonly-special hole, and the missing-target no-op, all at the edges of the slot's own two discoveries). 124 pins (31+17 red-on-tip/base + mutation demo); lookup hot path FASTER than base (-2.7% micro). Gate 19,695 expected; compare-bash 2,986 EXACT; conformance 2,458; mypy 266.

## 0.738.0 (2026-07-19) - Boundary campaign W3: one iterative pattern relation (W-phase complete)
- **One compiled pattern engine serves every consumer** (Boundary Integrity Campaign, package W3 [BOLD], closes #20 H7 + the E-family and completes the W-phase): `PatternCompiler` consumes W1's protected `FieldRun` values DIRECTLY (the interim bracket-escape encoding is deleted), and the engine exposes exactly four relations - `full_match`, `matching_ends` (prefix removal), `matching_starts` (suffix removal), `matching_spans` (substitution) - behind per-consumer `MatchProfile` policies. Pathname expansion layers slash-component and leading-dot policy OVER the engine; case, `[[ ]]`, parameter removal/substitution/case-mod, HISTIGNORE, `help`, and `print -m` all route through the same relations (the "name filter" exemption loophole is closed). The regex-translation matching path (`PatternMatcher`, `shell_pattern_to_regex`, fnmatch uses) is DELETED.
- **The H7 defect family is dead**: the classic exponential-backtracking case (`a*a*a*...b` against long strings) went from TIMEOUT on base to instant (and 14% FASTER than base on the adversarial corpus); deep-literal patterns no longer raise RecursionError (clean to 20,000 chars); and after the verification bounce, star handling is fully ITERATIVE (two-pointer backtrack; 50,000-star patterns now match exactly as bash and base do). Bonus fixes: quoted class-special characters inside active bracket classes are literal members everywhere (W1 carry), and a pre-existing `nocaseglob` character-class folding bug. At the pinned extglob nesting bound psh fails with a clean shell error where bash 5.2 SEGFAULTS.
- **Verification bounce round** (9th; 21/21 campaign blockers real, 0 false): the integrator's conditional ruling on the memoized-recursion design reserved an adversarial-pattern attack - it fired (star count consumed recursion frames), forcing the iterative rework; the unpinned HISTIGNORE/help fnmatch-to-engine behavior change (bash-correct direction, live-interactive-bash-probed) gained red-on-base pins and an archived oracle transcript. HISTIGNORE now honors extglob-when-set and backslash escapes as bash does.
- Perf (base vs final): realistic mix +2.4%, pattern-saturated micro +20.0%, adversarial corpus -13.7% (faster). Gate 19,572 expected; compare-bash 2,986 EXACT; conformance 2,376 EXACT; mypy 265. Deferred census deletion (dead regex converters incl. `normalize_bracket_expressions`) routed to the Q-phase.

## 0.737.0 (2026-07-19) - Boundary campaign W2: one subscript authority
- **Array subscript keying has ONE authority** (Boundary Integrity Campaign, package W2; closes reappraisal #21's SIGNATURE finding - assoc-array subscript keying was ONE feature implemented SIX inconsistent ways across six modules): `SubscriptEvaluator` (`psh/expansion/subscript.py`) interprets subscripts only AFTER lookup knows the target kind - indexed targets expand then lazily arithmetic-evaluate; associative targets get one quote-removal pass under a no-split/no-glob policy through W1's expansion funnels. Eight call sites route through it (assignment, lookup, unset, `test -v`/`[[ -v`, declaration, arithmetic lvalue, parameter expansion, initializer); the consumer-specific `int()` calls, raw quote-pair stripping, and per-module `assoc_key` copies are DELETED (12-item census). One renderer backs `declare -p`/`@A`/`@K` output.
- **21 of 26 keying divergences vs bash 5.2 fixed** (61-row probed matrix; the 5 residuals are classified outside keying: message wording, bash hash enumeration order, a pre-existing lexer extent). Composite quoting (`h['a''b']=v` and family Q1-Q8) now keys correctly; `unset "a[@]"` empties rather than removes; `unset "a[$empty]"` is bash's silent no-op; `test -v`/`[[ -v` arithmetic-evaluate indexed subscripts (`a[1+1]`, `a[i]`, `a[-1]`, scalar `x[0]`) including bash's fatal-abort on invalid subscript arithmetic. Probe-derived doctrine: in ARITH context the associative key gets quote/escape removal but NO `$`-re-expansion; arithmetic subscripts are verbatim, nesting-counted tokens.
- **Verification bounce round** (8th of the campaign; 18/18 blockers real, 0 false): caught the `test -v` indexed arm still holding a private `int()` with false parity claims, and a mutation-proven unpinned diagnostic-prefix change - both fixed, integrator re-probed 8 fix classes three-way (bash/base/branch) before shipping. `SubscriptUse` now genuinely drives the empty-subscript policy per consumer.
- Carries: subscript-procsub read-time gap RE-ROUTED to parser scanning with new discriminating evidence; sq-inside-dq plain spellings closed; operand-`$@` flattening declared with flip-when-fixed pins; empty-arith-subscript warn-continue divergence documented -> R-phase. Gate 19,527 expected (+161); compare-bash 2,986 EXACT; conformance 2,376; mypy 265.

## 0.736.0 (2026-07-18) - Boundary campaign W1: ExpandedWord field IR and field-splicing algebra
- **Word expansion produces a typed field IR** (Boundary Integrity Campaign, package W1 [BOLD], opens the W-phase; closes #20 H5+H6): `ExpandedWord(fields)` -> `ExpandedField(runs)` -> `FieldRun(text, protection, split, origin)` built by `WordExpander.expand_to_word()`; `materialize()` is the sole terminal IR->argv boundary. The `str|list[str]` union returns, the `$@` shortcut path, and the quoted/unquoted walker forks (`_expand_double_quoted_word`, `_expand_at_with_affixes`, `ExpandedSegment`, `_WalkState`) are DELETED - one field-splicing algebra covers `"$@"`, array `[@]`, explicit empties, IFS splitting, and affixes (first field joins the open prefix, middles commit, last stays open).
- **27 bash divergences fixed across five families** (all red-on-base pinned): quoted `"$@"`/`"${a[@]}"` affix splicing, per-character protection (`a\*b*` globs only the unprotected `*`; pathname patterns bracket-escape PROTECTED metacharacters), combined splice+protection rows, embedded unquoted `$@`/`${a[@]}` affixes under custom IFS (base joined fields wrongly), and nonadjacent-extglob eligibility (protected text between active runs breaks the operator, as in bash).
- **Adversarial verification earned its keep - BOUNCE round**: the 4-agent pass caught a branch-introduced glob regression the whole green gate missed (a no-match guard swallowed a legitimate match when a file is literally named like the pattern - nullglob vanished it, failglob errored; the guarded case was provably unreachable), plus the unpinned embedded-IFS class and dangling architecture-doc pointers. All fixed; no-match is now a distinct empty result; integrator re-probed three-way (bash/base/branch) before shipping. Scorecard: 16/16 blockers real across the campaign, 0 false.
- Rulings: `FieldRun` slots-not-frozen sanctioned (allocate-fresh-never-mutate proven by mutation test); ~3% saturated end-to-end perf cost accepted (clarity-over-performance charter). Gate 19,366 expected (+93); compare-bash 2,986 EXACT; conformance 2,280; mypy 264.

## 0.735.0 (2026-07-18) - Boundary campaign S5: pipeline components and exhaustive traversal (S-phase complete)
- **A function definition is an ordinary pipeline component** (Boundary Integrity Campaign, package S5; closes #20 H9): `FunctionDef` joins the `PipelineComponent` sum (Statement AND Command), so `f(){ } | cat`, `f(){ } &`, `! f(){ }`, `time f(){ }`, and defs in `&&`/`||` lists parse in BOTH parsers exactly as bash's grammar allows. The H9 divergence was a PURE GRAMMAR GAP - 10 formerly-parse-error rows now match bash's leak/rc/stdout semantics across -c/file/stdin/eval/source with ZERO executor changes (psh's execution contexts were already bash-correct: single-member runs in-process and leaks, real pipelines and background fork and don't). A standalone def keeps its historical bare AST shape byte-identically (Option A, sanctioned).
- **`walk_ast` over a declared `AstChildSchema` is the sole structural traversal**: AST child fields are schema-declared with a drift-lock guard (schema vs annotation reflection, offender-proven), a totality self-test, and `iter_child_nodes` rewired as a thin alias. LATENT BUG FIXED: the old reflection walker silently skipped `IfConditional.elif_parts` - the traversal-consumer corpus (debug-ast, formatter, validators, linter, security, metrics) is byte-identical except the now-correct elif metrics. S3 templates stay non-children by construction (matching the `CommandSubstitution.program` opt-in precedent), guarded three ways.
- Verified by a 4-agent adversarial pass: PASS, ZERO blockers (fifth clean pass), 9 nits dispositioned (incl. pins for the 3 still-invalid def-continuation diagnostics and a sanctioned-exception record for the pre-existing `_offset_line_numbers` reflection walk, carried to the Q2 ratchet). Gate 19,273 expected (+146); conformance 2,243; mypy 264.

## 0.734.0 (2026-07-18) - Boundary campaign S4: honest parser calls and resumable-ready outcomes
- **The parser-call contract is typed** (Boundary Integrity Campaign, package S4): `ParserContext` now composes a frozen **`ParseInputs`**(source_text, line_offset, lexer_options, heredocs, config) with a mutable per-call **`ParserState`**(cursor, nesting_depth, substitution_depth, open_constructs) - flat accessors preserved as delegating properties, sole construction sites guarded. Both parsers expose a typed **`ParseOutcome = Complete | Incomplete | Invalid`** via `parse_outcome()` over the single `outcome_from_parse` classifier; the raising `parse()` remains the terminal adapter (executor call sites untouched). The scripting accumulator and cmdhist joiner consume the typed `Incomplete`.
- **Parser instances retain nothing after return**: the combinator's instance-retained per-call heredoc map (S2 handoff) is absorbed into `ParseInputs`, installed at parse start and cleared/restored at its three guarded entry points; a post-parse snapshot guard + synthetic offender prove it. The RD tree's only `finally` is the balanced nesting counter.
- **Three combinator algebra laws were VIOLATED on base and are fixed**: `optional` preserves committed failure, `then`/`map` preserve the failure position, `many` rejects zero-width success - pinned as property-style law tests with red-on-base transcripts (inert on the current grammar: base-vs-fixed error differential byte-identical).
- **Error carets compute against ONE coordinate system**: `(line, column)` in the original source (bash-aligned), replacing the mixed body-bearing/stripped-offset carets (S2/S3 handoff closed); the combinator now threads `ParseInputs` so its templates carry RD-equivalent line offsets and lexer options, and the previously-RD-only template attachment pins parametrize both parsers.
- Handoff finding: the `$'E\nF'` heredoc-delimiter-in-cmdsub case is NOT a divergence - bash also gives rc 2 nested (the earlier "rc 0" note was the top-level EOF-delimit case); pinned rc+stdout parity. Verified by a 4-agent adversarial pass: PASS, ZERO blockers (fourth clean pass), 8 nits dispositioned incl. a disclosed-divergence pin for the combinator's Invalid-vs-Incomplete classification of unclosed expansions (no live surface affected; carried with I3). Gate 19,127 expected (+148); mypy 264 files.

## 0.733.0 (2026-07-18) - Boundary campaign S3: structured syntax with bash-correct timing
- **Syntax-bearing regions carry typed templates** (Boundary Integrity Campaign, package S3 [BOLD]; #20 H2 + the S1 `$(in)` handoff): frozen `WordTemplate`/`ArithmeticTemplate`/`SubscriptSpec` (`psh/ast_nodes/syntax_templates.py`) attached by BOTH parsers to parameter-expansion operands, `$(( ))`/`(( ))`/C-style-for clauses, and array subscripts. Each template holds the raw region `text` (the designed lazy-grammar authority - bash re-parses expanded text) plus `NestedSub` entries whose modern `$()`/`<()`/`>()` bodies carry read-time-parsed `Program` nodes.
- **Bash's read-time nested-grammar validation, with lazy arithmetic preserved**: `echo ${x:-$(if)}` now rejects the whole buffer at parse time in the direct channels (dead branches included, all operand operators, subscripts, arith templates, nesting depth >= 2) exactly as bash does - while dynamic arithmetic (`op='+'; echo $((1 $op 2))`) keeps working because each region's OWN grammar stays lazy. Single-quoted bodies stay literal; ALL backticks stay deferred behind the named `NestedSub.is_deferred_backtick` seam (non-fatal, empty expansion, execution continues - probe-pinned).
- **Typed I3 producer contract**: substitution-body syntax errors are raised as `SubstitutionSyntaxError` (a behaviorally-inert `ParseError` subclass) at the ONE chokepoint `parse_nested_command`; psh keeps its uniform rc 2 (bash's 127/frame-abort string-channel mapping is carried to package I3, which will consume the tag). Eval/source fatality probed as command-substitution-specific and routed with it.
- Verified by a 4-agent adversarial pass: PASS with ZERO blockers (third clean pass of the campaign); 16 nits dispositioned (evidence rescue, ledger baseline corrections against the committed attestation, a valid-procsub accept pin added as integrator fixup). 97-row SHA-stamped timing battery; base differential 12 stdout DIFFs -> 1 (the residual is W2's subscript keying, now fed a faithful `index_spec`); 3 mutation batteries red; +236 pins net (gate 18,743 -> 18,979 expected).

## 0.732.0 (2026-07-18) - Boundary campaign S2: one heredoc transaction
- **Heredocs are a typed transaction, not string glue** (Boundary Integrity Campaign, package S2 [BOLD]; #20 H1+H3, r21 G1): frozen `HeredocSpec(id, raw, cooked, quoted, strip_tabs, span)` built by ONE constructor (both delimiter scanners and the `$(...)` extent scanner route through it), `CollectedHeredoc`, and an immutable `LexedUnit(tokens, heredocs)` lexer/parser boundary. Attachment is id-keyed at construction - the string `heredoc_key` surface, both parsers' quoted heuristics, the accumulator's private close policy, and the combinator's post-parse HeredocProcessor walk are DELETED (17-entry census).
- **FIFO collection closes the out-of-order bug family**: only the head of the pending queue can match a terminator - `cat <<A; cat <<B` with body lines equal to later delimiters now collects exactly as bash (both completeness oracles delegate to the one head-of-queue policy; red-on-base permutation battery, file/stdin/-c modes).
- **Delimiters cook through the one quote-removal machinery**: `$'EOF'`/`$"EOF"` arms added (base decoded `$'EOF'` as `$EOF` and ate the terminator plus the following command); the formatter emits the RAW spelling - `<<$X` round-trips as `<<$X` (base emitted the cooked form). A latent base bug fell out: post-lex fusion sliced body-bearing input, corrupting fused values after heredoc bodies.
- Verified by a 4-agent adversarial pass: ALL behavioral claims held; bounce was two doc-drift blockers (retired-representation sketches in live docs outside the guard scan scope) - fixed as integrator fixups, with the combinator heredoc routing SPY re-homed into the parity suite. 62-execution battery (18 unique red-on-base rows) green; AST differential over 1,509 sources clean; 92 new pins.

## 0.731.0 (2026-07-18) - Boundary campaign S1: complete lexical words before keyword classification
- **A word is fused before it is classified** (Boundary Integrity Campaign, package S1; #20 mediums 1-2): the lexer pipeline is now fusion-first - `word_fusion.fuse_words` builds the complete word (typed parts, per-part protection runs, exact source span) and the KeywordNormalizer promotes ONLY complete unquoted literal words. The keyword-prefix promotion (`then$x` -> THEN + expansion) is deleted; the old deliberate-divergence pin is re-pinned to bash red-on-base. This fixed a LIVE bug on main found by the battery: `while$x true; do ...` ran as an infinite `while true`.
- **Quoted and escaped keywords are never keywords** (`"if"`, `\if`, `i\f`, `$'if'` are command words, as bash), and **bare command-position `in` is a syntax error** like bash/dash - with the for/case subject positions and the `>/dev/null in` = 127 rule preserved (the three command-position machines untouched, per their non-unification essay).
- Riders (each bash-probed red-on-base): case patterns accept keyword names in both parsers (fixes a pre-existing combinator gap), TIME joins RD case-pattern keywords, `select` rejects invalid identifiers with bash's rc-1-and-continue, for/select subjects widened with byte-matching diagnostics.
- Evidence: 78-row battery x 2 parsers (25 red-on-base), token differential over 1,587 sources (all golden streams identical; 15 divergences all in predicted classes; no timing regression), 3 mutation demonstrations, 49 new pins.
- Verified by a 4-agent adversarial pass - second 0-blocker PASS of the campaign; evidence-provenance correction applied (clobbered base-stamped JSONs deleted; lesson: commit before re-running SHA-stamping harnesses).

## 0.730.0 (2026-07-18) - Boundary campaign F2: one process activation owner
- **Constructing a Shell mutates nothing process-global** (Boundary Integrity Campaign, package F2 [BOLD]; #20 H18, continuation finding B): new `psh/core/process_lease.py` - `ProcessLeaseCoordinator` with one active owner, LIFO `ActivationLease` nesting, competing-live-owner rejection BEFORE mutation, complete grant rollback, fork reset, and dead-owner GC. libc `setlocale` and the recursion-limit raise move from construction to activation; the headline pin (a second constructed shell changes nothing observable about the first - libc locale, module globals, signal dispositions, pattern behavior) is red-on-base.
- **Component leases under the active token**: libc locale, signal dispositions, and the std-fd baseline acquire/restore with LIFO misuse-raises; the trap lease aligned so cross-shell same-signal overlap is structurally unrepresentable; permanent `exec` redirects park CLOEXEC backups high and USER redirects into the parking range now RELOCATE the backup first (bash's model - the shutdown-clobbers-host-stdin composition was caught by verification, fixed, and pinned at the exact fds). cwd is recorded but deliberately never restored (process-owned, section 16; doc amended).
- **`Shell.shutdown(reason)` is the one idempotent cleanup path** - exit builtin, REPL EOF, startup failure, and source completion all funnel through it (census-ratcheted). REPL EOF now exits with the last command's status, as bash (probe-backed rider).
- Static ratchets forbid `setlocale`/`setrecursionlimit`/`signal.signal` outside lease acquisition (alias-aware scanners, synthetic offenders).
- Verified by a 4-agent adversarial pass (1 bounce: the parking-range composition), focused re-verify with fresh inverted-order compositions; 102 F2 pins, broadest targeted coverage of the campaign (14,191/0 xdist + 662/0 serial + full conformance/compare-bash/PTY).

## 0.729.0 (2026-07-18) - Boundary campaign F3: one sourced-program service
- **Every program source enters parsing through one typed boundary** (Boundary Integrity Campaign, package F3; continuation mediums 2 and 5; F1 handoffs): new `psh/scripting/program_source.py` with frozen `ProgramSource` (six-channel policy table: NUL/byte policy, bang-eligibility, dangling-continuation, posix-syntax-exit, return-stops-file) and `execute_sourced_file(SourceRequest)` — the ONE sourced-file executor owning source depth, positionals, `FunctionReturn`, and RETURN traps. `source`/`.` and rc loading share it: rc is no longer a second dialect (`return 7` in an rc ends it silently with the pre-return status, as bash).
- **NUL/invalid-byte policy decided once, probed to the live oracle**: stream channels delete every NUL; sourced files run bash's pair-survivor rule (isolated NUL deleted, adjacent pair truncates) with binary refusal at MORE than 256 deleted NULs (live-probed at the 256/257 boundary); the script-channel shebang sniff follows the PATCHED 5.2.26 rule (NUL before the second newline only) - the unpatched-tarball rule was implemented first, caught by verification as a live divergence, and the tarball-vs-patched trap is now documented at the sniff itself. C citations are commentary; resolved-oracle probes are the authority.
- **Positional persistence honors bash's function-context gate**: `set --` inside an args-passed source persists only at variable_context 0 - a source inside a function restores the function's positionals (verification-caught composition).
- Sourced files never history-expand (even under interactive family); `-i script.sh` still does - the channel distinction the ProgramSource kinds exist to carry.
- Verified by a 4-agent adversarial pass (1 bounce: the two catches above), focused re-verify with fresh oracle probes; 110 new tests, red-on-base 37+5 demonstrated at both stages.

## 0.728.0 (2026-07-18) - Boundary campaign F1: InvocationConfig before Shell
- **The invocation is decided before the shell exists** (Boundary Integrity Campaign, package F1; continuation finding A, #20 H17, medium 1): new `psh/invocation.py` with pure `parse_invocation(argv)` returning a frozen `InvocationConfig`; `parse_args` deleted; `sys.argv` is read at exactly one entry-point call (AST-ratcheted). Rc files now observe the FULLY-configured shell (options, parser, positionals all applied first); an invalid parser exits 2 before any rc executes.
- **Interactive-family independence** (#20 H17): `-ic` and `-i script` perform interactive-family startup (rc, history load, expansion enablement) independently of the command source. Bash-probed corrections along the way: script lines under `-i` are recorded in-session AND the history file is loaded for any non-command source, so `!!` resolves exactly as bash (caught by a post-verification integrator pin; narrow bounce).
- **Registry-derived CLI surface** (medium 1): the hardcoded `euxvnfCB` string is dead; `-a -b -h -m -E -T -H` arrive via the option registry; `-h` is bash `hashall` (help via `--help`); histexpand's `short_flag "H"` closes reappraisal-21 CORE-4 on both the invocation and `set +/-H` surfaces. Sign policy corrected against bash ground truth (the brief's assumption was wrong): `+i` cancels interactive; `+s`/`+c` are sign-accepted-but-blind; bare trailing `-o`/`+o` print the set-o listings (rc 0) and continue.
- Construction is startup-free: rc/history moved to an explicit idempotent `run_invocation_startup()`; subshell clones never repeat startup. (Construction-time setlocale and the recursion-limit raise are recorded F2 handoffs, per the merge train.)
- Verified by a 4-agent adversarial pass (first 0-blocker PASS of the campaign) + one narrow post-verification bounce from an integrator pin; 59-row bash-compared invocation matrix + construction-order pins, 42/76 original pins red-on-base.

## 0.727.0 (2026-07-17) - Boundary campaign: Phase E CLOSED (seeded-census exit)
- **Phase-E exit met and documented** (`docs/boundary_phase_e_exit_2026-07-17.md`): three full gates under shuffle seeds 101/202/303 at one SHA, each 18,347 passed / 0 failed, with IDENTICAL collection and outcome censuses (19,931 collected ids); live-bash comparison 2,986/0; conformance 1,893/0; ruff and mypy clean. Transcripts SHA-256-hashed in the committed manifest.
- **The exit criterion earned its keep on its first run**: seed 101 exposed that the new process-state hermeticity pins (v0.726.0) assumed pytest definition order - the first observed interaction between the two Train-0 packages (E1's deterministic shuffle x E23's pollute-then-observe pairs). The pins are redesigned to be order-independent: every test asserts its entry state equals the module-import baseline, then pollutes; any execution order proves the fixture's restore, and no order can make the pin vacuous.
- The evidence-first doctrine is now fully in force: no semantic package (invocation, syntax, expansion, runtime, input, jobs) started before this close; release tagging is attestation-gated; all bash differential execution routes through the one oracle runner; the gate classifier cannot bless a nonzero exit.

## 0.726.0 (2026-07-17) - Boundary campaign E2+E3: one oracle runner; hermetic locale and process state
- **One typed bash-oracle runner** (Boundary Integrity Campaign, package E2; continuation findings G + medium 12): new `tests/harness/shell_oracle.py` with `resolve_bash()` (BASH_PATH -> Homebrew -> PATH, version-recorded) and `run_shell_case()` returning `Completed | SpawnFailure | Timeout | DecodeFailure`. The runner owns process-group sessions with kill-on-timeout and an orphan sweep, byte-capped file-backed capture (a cap breach kills the group - the 80GB self-feeding-cat incident is now structurally impossible), per-case temp cwd, UTF-8+surrogateescape, and a hermetic env builder (strips all LC_*/LANG and DISPLAY). Harness failures are typed and rejected BEFORE comparison: two identical crashes can never classify IDENTICAL (synthetic-offender self-test proves it). All 73 bash-oracle offender files migrated (the hardcoded no-fallback trio included); `find_bash` deleted; a new tooling ratchet with an EMPTY test-file allowlist forbids bypasses.
- **Locale truth** (package E3; continuation finding H - the root cause of the three env-sensitive gate failures): the PEP 538 phantom-LC_CTYPE strip in `psh/core/state.py` now applies only when coercion is PROVABLE; a nonempty `LC_ALL`, `PYTHONCOERCECLOCALE=0`, or explicit UTF-8-mode requests mean the inherited value is kept, matching bash. Pre-registered 13-row bash-5.2 matrix (both branches demonstrated red-on-base); 15 pins across unit + live-bash conformance; user-guide paragraph corrected.
- **Test hermeticity**: suite-wide autouse fixture snapshots/restores signal dispositions, std fds 0-2, cwd, env, and the process libc locale per test (the locale arm pinned red-on-base). The serial-phase contaminator behind the 36 isolation-dependent fatal-signal failures was NAMED and fixed at source: not signal state but a `pgrep -P` readiness probe in `test_exit_trap_paths.py` that killed already-exited shells; replaced with in-band sentinel readiness that raises loudly. The historical r18t2 `/dev/stdout` agreement was capture-mode-dependent (bash's tempfile+rename history write silently fails into /dev) - case reclassified with archived probe.
- Verified by a 4-agent adversarial pass (1 bounce: a silently-dropped locale-restore clause and a false migration claim, both fixed and spot-checked). +48 tests vs v0.725.0.

## 0.725.0 (2026-07-17) - Boundary campaign E1+E4: structured gate results and same-SHA release attestation
- **The gate can no longer bless a failure** (Boundary Integrity Campaign, package E1; reappraisal-20-continuation finding F): `run_tests.py` now consumes structured per-phase JSON manifests (collected node ids + all seven outcome counts, written by the new `tools/pytest_phase_manifest.py` plugin, xdist-truthful) instead of parsing transcript text, and NO nonzero pytest exit is ever translated to success - the historical xdist-teardown-race carve-out is deleted (a race now means a red gate and a rerun, never a reclassification). Worker loss, INTERNALERROR, missing/truncated/inconsistent manifests, and empty collection all fail independently of the summary text. The adversarial counterexample that defeated the old classifier (benign-race line plus an unrelated INTERNALERROR under a clean summary) is pinned red-on-base.
- **Performance tests rejoin the tree honestly**: the global `--ignore=tests/performance` is gone; deterministic robustness tests run in the standard gate, timing benchmarks carry a `benchmark` marker and run via the new `run_tests.py --benchmarks` serial tier (wired into nightly as an artifact; baseline-delta tracking deferred to campaign exit). `TestMemoryUsage` renamed to what it actually is (`TestLargeInputRobustness`).
- **mypy coverage hole closed**: the bare `psh.parser` override reached only the package `__init__`; now `psh.parser.*`, with a new guard proving every psh module resolves to `check_untyped_defs = true` (the bare-vs-star glob semantics reproduced empirically in the guard's self-test).
- **Same-SHA release attestation** (package E4): a green standard gate can write `gate_attestation.json` (`--write-attestation`; refuses non-green runs and dirty trees via lossless `git status --porcelain -z` parsing); `release-tag.yml` now verifies - before tagging - that the attestation's version matches, its gated commit is an ancestor, and NOTHING but the attestation changed since the gate ran. A version bump without valid attestation fails the workflow loudly instead of tagging silently.
- **Tooling**: `--shuffle-seed N` (deterministic collection shuffle, for the Phase-E identical-census exit); canonical lint is now `ruff check psh tests tools`; `sys.executable` replaces bare `python`; `deselected` no longer inflates the combined summary; `testing_source_of_truth.md` rewritten (the fictional "subshell phase" is gone).
- Verified by a 4-agent adversarial pass (1 bounce cycle: a porcelain-parse defect in the attestation writer and a stale-evidence probe, both fixed and re-verified). 69 -> 72 targeted guard tests; +45 tests vs v0.724.0.

## 0.724.0 (2026-07-12) - Reappraisal-19 P7: measured efficiency residue (campaign finale)
- **Every change measured before/after, and one measured OUT**: the RD parser's per-call frozenset unions become named TokenGroups members (-88%% on the union micro); the lexer's per-token sorted() becomes a class-level _OPERATOR_LENGTHS derived from OPERATORS (drift-proof - a new operator length is picked up automatically); KeyDecoder's pending-bytes queue becomes a deque (-39%%); and the set -x for-loop header string is built once per loop instead of per iteration (-43%% end-to-end on a traced loop; a 14-case three-way battery proves xtrace output byte-identical - PS4 still re-expands per emitted line, exactly like bash). The brief's CommandAccumulator prefix-caching item was SKIPPED on the evidence: preprocessing is 0.21ms of a ~9.8s parse-dominated budget - the quadratic bound is documented in the docstring and backstopped by an off-gate perf guard instead.
- **The one sanctioned behavior delta**: the interactive undo stack is bounded at 1,000 snapshots (deque maxlen; was unbounded growth), red-on-base pinned.
- Campaign note: this slot produced the 13th and final adversarial-verify bounce - a new species (the ledger cited the briefs doc for latitude phrases that came from the integrator's slot instructions; real instruction, wrong provenance). The fix codified the rule: quote only with a pointable source.

## 0.723.0 (2026-07-12) - Reappraisal-19 P8: the blind-spot monoliths get read, and the walks converge
- **The two zero-finding monoliths were read end-to-end** (appraisal M16 - the blind spot was the point): `ScopeManager.set_variable` earns a COHESIVE-AS-IS verdict (its length is an ordered precedence ladder - nameref -> temp-env -> dynamic-special -> scope-cell - whose interceptor order is the behavior; both analyses live in the campaign ledger), while `execute_function_call` earned a decomposition: a new `_function_frame` context manager makes the six-piece save/restore STRUCTURAL (a saved field can't be added without its restore; mutation-demonstrated - deleting one restore fails the suite loudly). 25-case battery byte-identical across base, pre-refactor, and final.
- **Three enumeration walks converge on one `iter_effective_variables()` generator** - and the brief's fourth was proven to deserve its independence: `sync_exports_to_environment` carries exported-instance-per-name semantics (an exported outer feeds the child env under a non-exported `local +x` shadow - live-bash-probed both ways), so converging it would have been a regression. The exception is documented at both sites.
- **`export -p` now lists exported SHELLOPTS/BASHOPTS** like bash (the one behavior change, red-on-base pinned); `state.variables` is docstringed as SNAPSHOT semantics (T13 had already migrated all four single-value production readers).

## 0.722.0 (2026-07-12) - Reappraisal-19 P6: the strict-errors borders stop lying about what can happen
- **Arithmetic cant-happen branches raise RuntimeError("internal: ...")** (4 live sites; the appraisal's fifth was merged away by T9's LValue reification): under the suite-wide strict-errors regime an impossible branch now fails LOUDLY instead of masquerading as a user arithmetic error. The ValueError catch is KEPT - the appraisal's "narrow the catch" prescription was proven wrong (a >4,300-digit integer parse is a USER-reachable ValueError) - and both directions are pinned with mutation-proven discrimination: the pins anchor the exact primary message shape, so silently falling through to the 'unexpected arithmetic error' fallback now fails tests (a verify-layer catch; the substring-matched both shapes before).
- **read/mapfile get a typed error dialect**: ReadError(PshError) carrying its rc replaces read's ValueError-plus-setattr('rc') convention and mapfile's private _OptionError - byte-identical user output (30-case battery, empty diff base-to-tip).
- **UnclosedQuoteError roots at PshError** via dual-inheritance (PshError, SyntaxError) after a 9-site catch audit found two load-bearing bare `except SyntaxError` consumers (the brief's documented fallback); exceptions.py's "everything roots at PshError" claim is now true. internal_errors.py's guard-count docstring de-numerized (it undercounted 7 delegates as "four") - and the same undercount fixed in core/CLAUDE.md.
...
- **175 lazy imports hoisted** (380 -> 205 deferred; the four hotspot files drop to 3/0/1/0), each proven by import-success AND no-new-package-cycle - the two-part criterion caught one hoist that imported fine but created a fresh executor<->interactive cycle. Genuinely layering-forced lazies carry `# cycle-break:` comments naming their cycle.
- **THE LOCK** (`tests/unit/tooling/test_import_layering.py`): AST-builds the module import graph and asserts zero package cycles (allowlist EMPTY), psh/utils leafness + psh/core near-leafness, and a down-only per-file ratchet on deferred imports - it fired on arrival, catching a hoistable lazy import that rode in with the P5 merge (fixed by hoisting, per its own prescription).

## 0.719.0 (2026-07-12) - Reappraisal-19 P1+P2: the docs stop teaching fixed bugs
- **Contract-docstring truth sweep** (appraisal H19): mode-aware persistence wording at the seven executor sites (the appraisal itself had gone stale - the F9 fix made prefix-assignment persistence conditional, so blind transcription would have written the OPPOSITE falsehood); executor/CLAUDE.md strategy order, designations, and last_exit_code all match the code they describe.
- **Six subsystem CLAUDE.mds re-synced against HEAD** (lexer, parser, core, builtins, io_redirect, interactive): drifted sketches replaced by invariant prose + file#symbol pointers - env-writes via the observer, ShellOptions, the T3 write()/error() trio, fd-backup and heredoc recipes, {var}>file named-fd paths (all four callers enumerated), lazy self-pipe, typed jobspec API. THE RULE is now in root CLAUDE.md: subsystem docs carry invariant prose and pointers, never implementation sketches - every embedded sketch the appraisal examined had drifted into teaching a fixed bug.
- **Sketches that stay are drift-locked**: new `test_doc_snippets.py` asserts registered doc snippets appear verbatim in their source files (4 entries, guard-the-guard self-tested). One adversarial-verify bounce, fixed with a probe: the truth sweep itself had shipped a fresh falsehood ("macOS lacks WCONTINUED" - it doesn't; the real v0.661 quirk is that no SIGCHLD is DELIVERED on continue). In a docs-truth slot, platform claims are code claims: probe them.
- Riders: adopt->clone_for_child rename completed (test files renamed to test_state_clone_for_child.py / test_state_clone_completeness.py; every live citation swept); phantom-method docstring fixed in file_redirect.py; arithmetic package docstring tightened.

## 0.718.0 (2026-07-12) - Reappraisal-19 P5: help finally has an oracle
- **Spec-to-help sync is now a meta-test** (`test_builtin_help_sync.py`): for every builtin with a declarative `parse_flags` spec (18 post-T3), both directions are asserted - every spec'd letter appears in the help/synopsis, every advertised flag is in the spec. 17 static + 1 dynamic (ulimit, static part still checked); ONE justified allowlist entry (`jobs -x`, dispatched before parse_flags). The oracle caught three drifted builtins red-on-base on arrival (jobs -n, pwd -L/-P, unset -v/-n).
- **Known synopsis drift fixed by DERIVATION, not transcription**: shopt's help list renders from the SHOPT registry (was hand-synced at 8 of 11), the debug builtin's option map derives from the option registry's DEBUG category (phantom 'parser' row deleted; three pasted copies deduped), `help`'s version line reads `psh.version` (hardcoded '0.54.0' gone), and `explain_parse` shrinks to a summary derived from the actually-wired modules (stale 'Key advantages' blurb deleted, negatively pinned). Plus: unset/declare/local/read/exec synopses, the two `-h`-prints-`__doc__` surfaces now print real help, trap's `help_text` renamed to the standard `help`, and `python -m psh --help` documents `-B/+B`, `-o/+o`, `+x`.
- **User-guide help transcripts regenerated** from the current builtins (the T3 carry: cd/wait/dirs plus their quick-ref neighbors), so the guide shows what the shell actually prints.

## 0.717.0 (2026-07-12) - Reappraisal-19 B3: function bodies through the compound chokepoint
- **One dispatch for compound commands** (appraisal H12): `FunctionParser.parse_compound_command` routes through `CommandParser._parse_compound_component` instead of its own hand-rolled if/elif + brace parse; `parse_control_structure` (whose dispatch chain existed only for this caller) is deleted. Byte-identical AST for all legal inputs - proven by a 53-case `--debug-ast` differential (including the redirect-placement crux: brace bodies keep redirects on `FunctionDef.redirects` with a bare StatementList body, subshell/control bodies keep them on the inner node).
- **Function bodies now count toward nesting depth** (the point of the change): ~1,200 nested function definitions raise the standard `MAX_NESTING_DEPTH` ParseError at depth 1,000 instead of parsing unboundedly; 64-deep legal nesting still parses AND runs. Deliberate psh divergence (bash has no such guard), same class as the arithmetic depth guard; boundary pinned at 1000/1001.
- **Chokepoint routing kept honest**: the LPAREN branch's `\$(`-guard (built for `echo \$(...)` argument position) is suppressed exactly on the function-body entry via an explicit `in_function_body` parameter - `function f\$ (echo hi)` parses as it always did (rc 127 at call, like bash), while `echo \$(date)` inside any function body still errors. Caught by adversarial verify; pinned with a 14-case escaped-dollar family red-on-the-bounced-tip.
- Rider: parser CLAUDE.md dispatch example + error-collection claim updated to the current shape (routed carry).

## 0.716.0 (2026-07-12) - Reappraisal-19 B4: visualization renderers converge on one reflection walk
- **One `node_fields()` helper** (appraisal H15): ascii_tree, sexp_renderer, and dot_generator all adopt the `__dataclass_fields__` walk ast_formatter already modeled, killing the ~80-line `dir()` reflection twins AND the phantom-field reads in one move (DOT's ForLoop/CStyleForLoop read nonexistent `iterable`/`init`/`condition`/`update` and silently dropped those fields from the graph - now rendered). Per-node visit methods survive only for labels/colors. ast_formatter's own output is byte-identical before/after.
- **`render()` is a real classmethod** (appraisal H16): `CompactAsciiTreeRenderer.render()` silently built the BASE class, so `--debug-ast compact` and `parse-tree -f compact` never rendered compact output - fixed via `cls(**kwargs)`, pinned red-on-base. `show_positions` now actually reads `node.line` everywhere it is offered (was a silent no-op in ascii/dot); sexp renders the actual and-or operator; DOT drops its bogus `(|)` pipe-label.
- **Drift-lock corpus**: 5 scripts x 5 renderer configs pinned as golden files (`PSH_UPDATE_VIZ_GOLDENS=1` regenerates for reviewable diffs) - future AST changes now break loudly instead of silently drifting the debug surfaces. `SExpressionRenderer` exported (production importer existed; the omission was the defect); production-dead `DetailedAsciiTreeRenderer` + `render_detailed_tree` deleted per rule 5.

## 0.715.0 (2026-07-12) - Reappraisal-19 B6: expand_history becomes one forward scan
- **The three backward rescans are gone** (appraisal H22): `expand_history`'s per-`!` backward rewinds (bracket / `${...}` / `$((...))` detection via triple-nested `while/else`, O(n) at every `!`) are replaced by a single forward pass. A module-level `_SuppressionContext` tracks bracket/brace/arith depths incrementally - fed every consumed character in order, including inside quotes, because the old scans were quote-blind and byte-compatibility demanded reproducing that (warts preserved by design: `'['` in single quotes still suppresses a later unquoted `!`). The `_resolve_*` resolvers are untouched; scanner and resolvers are now cleanly separated.
- **Characterization-first discipline**: a 2,576-case corpus (197 hand-enumerated covering every suite input + the appraisal's nesting/adversarial cases; 2,379 combinatorial) was frozen from the ORIGINAL code as its own commit BEFORE the rewrite, then asserted byte-identical after - plus an exhaustive differential harness (~5.3M legacy-vs-new comparisons, 0 divergences) and independent adversarial fuzz on fresh ground (long strings, UTF-8, quick-substitution, embedded newlines - 0 divergences). The corpus stays in the suite as a permanent drift-lock (collected count +2,576).

## 0.714.0 (2026-07-12) - Reappraisal-19 B5: the keyword-comparison guard finally guards
- **Born-vacuous meta-guard made real** (appraisal H21): `tests/unit/tooling/test_keyword_comparisons.py`'s regexes were double-escaped inside raw strings since birth - they required a literal backslash in scanned source and matched NOTHING. Single-escaped, and the receiver un-anchored from the literal name `token` (which production never uses) to any token expression, with the RHS restricted to the actual reserved words sourced from production's `KEYWORD_TYPE_MAP` - self-maintaining, and provably zero false positives.
- **The working guard immediately earned its keep**: it surfaced two latent raw keyword comparisons, both predating the appraisal, both fixed behavior-neutrally - `commands.py` time-prefix detection (`tok.value == 'time'` -> `matches_keyword_type(tok, TokenType.TIME)`) and `line_editor_helpers.py` for/in continuation (`tokens[0].value == 'in' and type in (IN, WORD)` -> `matches_keyword(tokens[0], 'in')`). Neutrality proven by AST differentials (byte-identical main vs branch) and a 6/6 bash-5.2 battery.
- **Guard-the-guard self-test added** (the idiom every sibling meta-guard has): synthetic offenders must be flagged, typed forms must not; mutation-proven load-bearing (double-escape regression, receiver re-anchoring, and offender reintroduction all fail loudly). Dead `.py.archived` allowlist entry deleted (triply-dead: file absent, glob can't yield it).

## 0.713.0 (2026-07-12) - Reappraisal-19 B1: patterns finally see newlines (the appraisal's last live-by-reading bug)
- **The glob-regex fast path is newline-safe** (appraisal H3): the two classic Python-re porting mistakes are fixed - converter output is wrapped in a scoped `(?s:...)` at both public entry points (so `?`/`*` match newlines like every shell glob should), and every end anchor is `\Z` via one idempotent `_end_anchored()` helper (bash's `$'a\nb'` cases no longer slip the anchor). `case $'a\nb' in a?b)` matches; `[[ $'a\nb' == a?b ]]` is true; `${x%b}` on `$'ab\n'` stops over-matching; `${x//?/-}` covers newlines. 44-case battery: base failed 23, tip 44/44 = bash. The memoized pattern_engine was already correct - the fast path has stopped drifting from it.
- Ruled riders: the `!(P)` negation-lookahead anchor (same bug class) and a bonus correction - `${x%$}` no longer over-matches a literal trailing `$` (the old rstrip-and-reanchor idiom's bug; bash-verified). Sibling-converter audit: pattern.py delegates, glob.py was already newline-safe - zero carries.
- 50-case non-newline byte-identity sweep: 49 identical, the one diff being the literal-`$` correction toward bash. One bounce cycle (a dropped compile-hoist sweep item, restored with a per-site verdict table and an honest ledger correction). 19 goldens (corpus 1,505 after union).
- With B1, all four live behavioral bugs the reappraisal-#19 audit found BY READING (readonly-local bypass, heredoc body execution, quoted-procsub execution, newline-blind patterns) are fixed and pinned.

## 0.712.0 (2026-07-12) - Reappraisal-19 T3: six builtins on one option walk; one PWD updater; one banner; one PATH walk
- **The option-parse chokepoint is real now** (appraisal H6): `parse_flags_ordered()` (argv-ordered events + an in-walk `check` validator hook so value errors keep bash's argv-order precedence) underlies `parse_flags`; read, mapfile, wait, trap, ulimit and cd all migrated onto it with bash-shaped invalid-option/usage messages via the v0.690 channels; print/kill/dirs stay hand-rolled with their reasons written down. Along the way: `wait -x` reports rc 2 + usage like bash (was rc 127), `-np`-style clusters parse, `wait -f` is accepted (bash parity, conformance-flipped per protocol), `wait -p 1bad` errors WITHOUT waiting (timing-proven), and ulimit's precedence follows bash's positional-for-absent-letters / post-scan-for--p rule (both probed).
- **One readonly-aware PWD/OLDPWD updater and one logical-cwd reader** serve cd, pushd, popd and dirs (appraisal's three-copy divergence, of which only cd had the readonly fix); the brief's 'dirs tracks $PWD' hypothesis was FALSIFIED by probes (bash tracks an internal cwd - implemented and pinned, brief annotated). The readonly battery is bash-exact including stack states; six probe-driven parity riders landed pinned (quoted-'~', operand-as-typed errors, cd - print order, dirs staleness, symlink-logical pushd).
- **One `type`/`command -V` banner** (probes disproved the suspected wording divergence - bash is identical across all six kinds, so no style knob); **one PATH walk** - `source` delegates to `CommandResolver.search_path(mode=os.R_OK)` with the empty-component rule fixed (`PATH=':/dir'` picks cwd like bash) and the builtins CLAUDE.md single-walk sentence true again.
- Process: dev-implemented across four sub-briefs; adversarial verification BOUNCED once (dropping the brief's check-hook silently regressed error precedence - restored per design, and the sweep surfaced two MORE latent precedence bugs, fixed and pinned); independently re-verified PASS with a timing proof. 41 pins + 12 goldens (corpus 1,486 after union); DirectoryStack.initialize retired under the test-only=dead rule.
...
- **Tab completion computes the word boundary once**: `get_completions(line, cursor) -> (word_start, completions)` replaces the vestigial-parameter signature; the three per-Tab `find_word_start` scans collapse to one (contract-pinned). Riders: `_get_key_action` is a one-line delegation; the `expand_prompt` naming collision is gone (`PromptExpander.decode_escapes`, rename mechanically proven assertion-free).
- Process: dev-implemented; 4-agent adversarial verification PASS with zero blockers (the no-behavior-change claim attacked via independent matrix reproduction, PTY sessions on both trees, and a thunk-isolation mutation check). No goldens needed - interactive-only surface, compare-bash untouched.

## 0.708.0 (2026-07-12) - Reappraisal-19 T11: one ANSI-C encoder; ${v@Q}, printf %q, declare -p and set finally agree
- **The three divergent `$'...'` encoders are one** (the appraisal's verified three-way output disagreement): `ansi_c_encode` (octal + named escapes + `\E`, the probe-verified bash-matching authority) moved to `utils/escapes.py` as THE encoder; `${v@Q}` and `printf %q` delegate their control-char bodies to it. For `$'\x01\x1b'`, psh previously rendered three DIFFERENT strings across the four reuse surfaces - now one bash-exact shape on all four (12 cases red-on-base; `declare -p`/`set` proven behavior-neutral, they already carried the authority shape; formatter idempotence byte-identical).
- escapes.py's famous "do NOT deduplicate" header now states its boundary explicitly: the rule covers the five DECODER dialects; the encoder is the opposite case - one authority.
- The high-raw-byte edge ($'\xff': bash `$'\377'`, psh printable U+00FF) is pre-existing byte-model territory, verified unchanged before==after on all four surfaces, and now explicitly cross-referenced as a deliberate divergence (M8 family) at the pin and in the campaign deferred ledger.
- Process: dev-implemented; 4-agent adversarial verification PASS with zero blockers (every committed pin independently re-derived byte-exact against bash 5.2). 7 goldens (corpus 1,474 after union).

## 0.707.0 (2026-07-12) - Reappraisal-19 T8: expansion twins converge; stored values are data, not code; substring arithmetic is lazy like bash
- **Stored variable values are no longer re-$-expanded by arithmetic** (the appraisal's never-rescan invariant, now real): `y=5; x='$y'; echo $((x))` errors like bash instead of printing 5 - with the change swept across every arithmetic-consuming surface ([[ -eq ]], let, write-subscripts, substring offsets), each pinned red-on-base.
- **The catch that made this release better**: the adversarial pass proved the first attempt REGRESSED a conformance case - bash evaluates substring offset/length arithmetic LAZILY (skipped when the subject expands to zero elements) and the old rescan had been masking psh's eager evaluation. Ruled fix: the operator went lazy, expand=False stayed. The probed rule - evaluate iff the subject expands to >=1 element (set-but-empty scalars ARE evaluated; $@/$* always; empty arrays are not) - is pinned by a 47-case battery, 16 lazy pins red on the bounced tip, and an independently re-derived 32-cell four-way matrix.
- **Twins converged (pure, byte-identical proofs)**: the three hand-rolled substitution scan loops are one loop with an explicit zero-width knob (both stepping semantics bash-evidenced); subscript splitting has ONE home (split_subscript, ~9 hand-rolled sites retired incl. the indirect-target shape test); the legacy expand_array_to_list string round-trip is gone; the evaluator resolves plain ${var} and $((...)) directly from AST components - expansion CLAUDE.md's 'never re-parsed at runtime' claim is finally true.
- **Efficiency with care**: case-mod predicates compile once per operator application (~3.5x on long subjects); the bracket-class regex cache is keyed on the ACTIVE locale ctype so v0.688's reactive LC_CTYPE switching can never serve a stale entry (probe-verified).
- Process: dev-implemented; bounced once on the eager-substring regression + four undeclared surfaces (the campaign's most substantive verification catch); re-verified with the rule matrix independently re-derived from bash. 28 goldens (corpus 1,467 after union).

## 0.706.0 (2026-07-12) - Reappraisal-19 T10: the analysis modes stop crying wolf
- **Seven verified always-fire/never-fire advisories fixed, each red-on-base** (appraisal H13/H14 + the MED cluster): the validator's useless-cat twin no longer fires on every bare `cat file` (deleted - the linter owns the check); `--lint` no longer reports every `FOO=bar` as an undefined function, and `set -eu` clusters are recognized; quoted `FOO="$@"` is no longer flagged; `(( i = i + 1 ))` is no longer a MEDIUM security finding (ARITHMETIC_INJECTION now requires genuinely injectable shapes); `cd -P /tmp` and `cd -- "$dir"` pass arity (operand counting, with bare `-` correctly an operand); `--metrics` finally counts process substitutions.
- **Three cross-visitor twins consolidated**: ONE `is_assignment` on the canonical SHELL_NAME (the hyphen-accepting stray is gone - `a-b=c` is a command word, not an assignment, discriminating-pinned per ruling); ONE predefined-variables set (`$HOSTNAME` now clean under both --lint and --validate); ONE unquoted-test-operand routine (union operator set - `[ $x -eq 5 ]` and `[ -z $x ]` now advised in both modes, positive+negative pinned). The orphaned TEST_OPERATORS union deleted; DANGEROUS_COMMANDS' false 'union' comment corrected with the linter caution table co-located and documented.
- **The drift-lock the sidecar never had**: tests/unit/visitor/test_analysis_advisories.py - a 31-snippet clean-corpus ratchet (idiomatic scripts must produce ZERO advisories across validate/lint/security) + positive/negative pins per advisory + twin guards; 93 tests. Bounce round added mutation-kill discipline: every discriminating pin's docstring names the mutation that kills it, with archived mutation-kill transcripts.
- Process: dev-implemented; 4-agent adversarial verification BOUNCED on two mutation-proven pin gaps (one pin was accidentally-green-on-base - the posixinv-wave attack dimension) - fixed with discriminating pins and independently mutation-demonstrated. Noclobber advisory rider dropped (was nagging every `>`); the brief's misattribution of its file recorded honestly in the ledger.

## 0.705.0 (2026-07-12) - Reappraisal-19 T9: the arithmetic engine reads like the textbook chapter it was meant to be
- **The 218-line operator elif ladder is a 57-line maximal-munch table** (3-char -> 2-char -> 1-char dict lookup; `++`/`--` keep their bash-pinned disambiguation path). Proven by an 896-expression differential - token streams byte-identical old vs new INCLUDING error parity across 193 error cases - independently regenerated by the verifier.
- **The lvalue is finally a concept**: `LValue(name, subscript, subscript_text)` + ONE `AssignmentNode` + ONE `IncDecNode` replace six scalar/array twin node classes, six twin evaluator methods, and three parser forks. Readonly enforcement now fires from exactly one `_write_lvalue` (the reappraisal-17 H1 ground - re-verified enforcing across scalar/indexed/assoc, messages byte-identical); associative literal-key semantics preserved via `subscript_text` (mirrors the retired node's contract, discriminator-probed).
- **Depth guards completed instead of a blanket catch** (integrator-ruled after the dev proved the brief's literal prescription RED - a blanket RecursionError catch mislabels function-nesting exhaustion as an arithmetic error; the discriminator test is mutation-proven): deep flat/`**` chains now yield a clean 'expression too deeply nested' rc 1 instead of a raw RecursionError (red-on-base), with the bash divergence (bash computes 25k-term chains) and the pathological composite-re-entry residue honestly recorded in the deferred ledger - the previous absolute narratives are hedged to what the guards actually guarantee.
- Process: dev-implemented; one bounce (a one-line dangling docstring to a deleted method + the narrative hedges) fixed comment-only; verified across two rounds (4-agent partial + focused completion after a session-limit interruption) - every dimension PASS, zero remaining nits.

## 0.704.0 (2026-07-12) - Reappraisal-19 T2: local converges on declare (the campaign's biggest twin healed)
- **`local` is no longer a 158-line divergent twin of `declare`** (appraisal H5): the shared declaration mechanics - ONE attribute-flag table with `-l`/`-u` cancellation, ONE array-init home (`build_array_init`, still argv-keyed via the shared `array_flat_text`), ONE nameref target validator, and the scalar-append-onto-array seam routed through v0.699's `compute_append_value` - now live in `declaration_engine.py`; `local` keeps only its `create_local` scope semantics (T1's readonly guard and multi-arg semantics preserved exactly, control-pinned).
- **Four carried divergences fixed, all bash-pinned red-on-base**: declare/typeset/readonly process every operand continue-on-error like bash; `readonly`'s diagnostics say `readonly:` (not `declare:`); the `+attr` removal grammar is parsed and enforced (`local +r`/`declare +r` REFUSE on readonly - closing T1's battery residual; `declare +i n=2+3` removes the attribute then assigns textually like bash); explicit `-a`/`-A` with `NAME+=scalar` appends onto element 0 instead of clobbering the array.
- **`local -n` adopts declare's nameref target-shape validation** (invalid targets rejected at declaration, function continues). The in-function self-reference case is pinned as a deliberate divergence (psh rejects; bash warns rc0) in the deferred ledger.
- Process: dev-implemented solo (the largest brief); 4-agent adversarial verification BOUNCED once - a declared-but-unset readonly local escaped the new +attr guard (only one branch of the symmetric fix was pinned), and a claimed pin proved absent under mutation - both fixed with a pre-registered tombstone matrix and independently re-verified PASS with every claim re-derived. 24 goldens + 1 upgraded (corpus 1,439 after union); 41 unit pins.

## 0.703.0 (2026-07-12) - Reappraisal-19 T6: one lex->parse pipeline (the third hand-synced copy retired); analysis modes stop lying
- **One heredoc-gate -> tokenize -> alias-expand seam** (appraisal H11): `psh/scripting/lex_parse.py` now owns the pipeline that was hand-copied (and drifting) across the trial-parse oracle, the execution path, and the analysis path. The analysis copy's two drifts are FIXED as pinned behavior changes: `--parser combinator --validate` now actually uses the combinator (was silently RD), and analysis receives the execution path's lexer options. Structural honesty preserved: the completeness oracle keeps its RD parser (it consumes RD-only incremental signals) but consumes the shared seam - the design is stated in the module docstring.
- **Analysis modes tell the truth now**: alias definitions earlier in the input are reflected in what --lint/--validate/--security/--metrics see (execution-truth), while `--format` deliberately preserves the user's source words (`expand_aliases=False` - a formatter is source-to-source; integrator-ruled with both branches pinned). Bare analysis errors gained the source-line caret; the `except (ValueError, TypeError)` swallow that hid visitor bugs as 'Error parsing command' is gone - internal defects now route through report_internal_defect and re-raise under strict-errors (red-on-base).
- **One syntax-error location renderer**: `_location()` dedups four hand-written ternaries; the UnclosedQuoteError clause routes through the standard renderer byte-identically; exec-path error shapes proven byte-identical throughout (script-file and stdin shape pins committed).
- Process: dev-implemented; 4-agent adversarial verification PASS with zero blockers; one integrator ruling refinement (the --format consequence) closed out with red-on-pre-ruling-tip evidence. 2 goldens + 12 convergence pins; one golden upgraded from psh_only to bash-compared (corpus 1,415, comparisons +1, skips back to 23).

## 0.702.0 (2026-07-12) - Reappraisal-19 T7: one child-exit taxonomy; one subshell body; one [[ ]] RHS walker
- **One child exception-to-exit-status taxonomy** (appraisal H10): `child_policy.map_child_exception()` + `CHILD_EXIT_EXCEPTIONS` replace five hand-copied maps across the fork sites; the launcher's `SystemExit(None)` -> 1 wrong-copy is fixed to Python's real semantics (-> 0, red-on-base pinned); pipeline's redundant per-member catch deleted (launcher coverage probed for loop-control and function-return exceptions in pipeline members). A tooling meta-test (with self-test) locks the taxonomy fingerprint to child_policy.py so a sixth copy can't accrete.
- **One subshell child body**: `child_policy.run_child_body()` is the shared middle of `run_child_shell` and the foreground `( )` executor, parameterized by exactly the deltas the appraisal's verifier enumerated (A/B smoke byte-identical).
- **One [[ ]] RHS walker**: the `_rhs_pattern`/`_rhs_regex` twin skeletons collapse into `_rhs_walk(escape, tilde)`; the thrice-pasted double-quote expansion recipe becomes `_expand_dquote_literal` (31-case quoting matrix byte-identical and bash-matching; three-site agreement pinned by a new conformance class).
- Riders (brief-sanctioned): select's PS3/menu, the out-of-loop break/continue announce, and the `[[` diagnostic now write through `state.stderr` like every other diagnostic (subprocess output unchanged and bash-matching; the in-process routing is pinned by a decoy-buffer discriminating harness added after the adversarial pass demanded it — red-on-base demonstrated); the dummy-SimpleCommand redirect idiom and the C-style-for arith-error mapping each get one helper (ledger records that reappraisal-17's full arithmetic-error chokepoint remains QUEUED, not absorbed - two `psh: ((:` sites are its candidates).
- Process: dev-implemented; 4-agent adversarial verification BOUNCED once (the stream rider lacked a discriminating in-process pin - the campaign's unpinned-behavior bar), fixed tests-only and spot-verified. 7 goldens (corpus 1,413 after union).

## 0.701.0 (2026-07-12) - Reappraisal-19 T5: redirect targets carry structure, not strings (procsub sniffing deleted); fd chokepoints converge
- **Process-substitution redirect targets are now carried as AST nodes end-to-end** (appraisal H1, two live bugs fixed red-on-base): `cat < '<(echo x)'` no longer EXECUTES the quoted literal as a command (it is a filename - ENOENT rc 1 like bash), and `cat < <(echo $x)` no longer double-expands the body via the legacy string fallback. The planner detects procsub structurally from the Word (`redirect_procsub_node`); `resolve_procsub_resource` consumes the node; the post-expansion string-sniff and the expansion fallback are deleted. Combinator-parser-mode pins included (mode-blind-pin lesson).
- **fd-behavior trio** (each bash-pinned red-on-base): `exec 0<&-; read x < f` reads the file instead of EBADF (stdin backup now uses the high-slot dup discipline the output path always had); the `<&-` close spelling is honored with direction-based defaults, so `echo hi 1<&-` reports the write error WITHOUT leaking 'hi' into the restored fd; the builtin `<` stdin path shares ONE file description (offset-sharing pinned; the last hand-rolled dup+fdopen now routes through `dup_sharing_stream`, gaining surrogateescape).
- **Chokepoints converge:** one `REDIRECT_OPEN_FLAGS` table behind every opener; `check_noclobber` at all three former hand-raise sites (one message); `setup_child_redirections`' three inline error shapes route through `format_redirect_error` (child pipeline members no longer leak raw `[Errno N]` reprs). io_redirect CLAUDE.md truth-repaired.
- Process: dev-implemented (relaunched cleanly after a session-limit interruption); 4-agent adversarial verification PASS with zero blockers; 14 goldens (corpus 1,406); two pre-existing divergences routed to the carries ledger (redirect-error location prefix for the errprefix program; {fd}> error naming).

## 0.700.0 (2026-07-12) - Reappraisal-19 T4: the heredoc rule family has one home (drifted delimiter twin healed; quote-blind detection fixed)
- **One heredoc delimiter-unquoting rule** (`unquote_heredoc_delimiter`, beside `heredoc_terminator_matches`): the three hand-synced copies had ALREADY drifted - `<<"A\B"` unquoted differently at two of three sites vs bash (and one inside-`$()` spelling was a hard parse error rc 2). All sites now delegate; bash 5.2's dquote rule nailed precisely (backslash escapes only `$`, backquote, `"`, `\`); 11-spelling battery 0 mismatches; the redundant fourth copy deleted outright.
- **Quote-aware heredoc detection** (appraisal H2, a live bug): a line like `echo '((' ; cat <<EOF ...` executed the heredoc BODY as commands on the full-buffer `run_command` path (quote-blind index-pairing opened a phantom arithmetic region; the `-c`/script/stdin paths were masked by the incremental accumulator - a mode-blindness catch). `_quote_flags` is now computed once per line and feeds the region scanners, so quoted `((`/`$(`/`${` can never open a region; `contains_heredoc` is reduced to an honest cheap gate. Bonus entailed fix: quoted `'<<EOF'` no longer suppresses the cmdhist join (bash-correct, PTY-oracled).
- **Two pure refactors, differential-proven**: the triplicated quote-lexing flow collapses to one `_lex_quoted` helper (637-case token-stream corpus byte-identical), and `KeywordNormalizer`'s two interleaved FSMs split into named components (1,367-case corpus x both entry points byte-identical).
- Process: dev-implemented from the campaign brief; the 4-agent adversarial pass BOUNCED the first tip - the weakened `contains_heredoc` leaked into the ONE caller using it as a final answer (interactive history joiner: arithmetic-`<<` compounds would have kept raw newlines and split into bogus entries on history reload). Fixed via an accurate-scanner walk (`_has_real_heredoc`), pinned with a real disk round-trip through HistoryManager, red-on-bounced-tip demonstrated, and independently re-verified PASS (no blockers, no nits). 8 goldens added.

## 0.699.0 (2026-07-12) - Reappraisal-19 T1: mutation authority has one home (readonly bypass closed; one += engine; one inherited-traps rule)
- **`local` can no longer silently overwrite a readonly local** (appraisal H7, probed red-on-base): a value redeclare (`local -r x=1; local x=2`) now raises the bash-shaped readonly error and the function continues; attribute-only redeclares correctly still merge (bash allows `local -i x` on a readonly local - the brief's both-paths sketch was corrected by probe, and the attrs-only path is pinned against readonly-stripping). Rider (ruled): `local`'s multi-arg loop is continue-on-error like bash - `local x=2 y=3` with x readonly reports on x and still sets y (also fixes the pre-existing outer-scope multi-arg abort).
- **One `+=` append engine** (H8): the integer/array/case-fold append formula now lives in exactly one place (`VariableStore.compute_append_value`); `resolve_append_assignment` is a two-line compute-only delegate (the temp-env/rollback callers need the value without a commit), and the store docstring's ALL-appends claim is finally true. Converged as a byte-identical characterization-first refactor, THEN a ruled behavior commit fixing four pre-existing bash divergences the matrix surfaced: temp-env prefix append leaked a literal `(5)+(3)` into child environments (now `8`); `declare -i n+=3` appended textually (`23` -> `5`); `local -i n+=3` likewise; and integer-append arithmetic now runs at bash's time (side-effect timing pinned). The scalar-append-onto-array-with-fresh-attrs branch is pinned too (verifier-caught gap; `declare -i a+=10` -> `11|2` matching bash).
- **One inherited-traps rule** (H9): `TrapManager.compute_inherited_traps()` - ERR exempt under errtrace AND DEBUG exempt under functrace - now feeds BOTH child-clone paths; previously the two copies disagreed and a `set -T` DEBUG trap survived into `{ ...; } &` children but not `( )` (13-case battery, all match bash).
- Process: dev-implemented from the campaign brief with three mid-slot integrator rulings (all probe-driven); 4-agent adversarial verification PASS with zero blockers; one verifier-demanded pin close-out (red-on-base against the refactor-only tree). 21 goldens added; compare-bash 2745/23.

## 0.698.0 (2026-07-11) - Reappraisal-19 Phase D complete: lexer and parser dead surface out; the phase retires ~1,500 net lines
- Phase D1 (final slot) of the reappraisal-#19 dead/duplicate-code campaign: net -517 lines across `psh/lexer` and the recursive-descent parser, closing Phase D. Phase totals across the six releases v0.693.0-v0.698.0: roughly 1,030 insertions / 2,540 deletions in production+tests+docs, all ~90 brief items disposed (deleted, text-fixed, or evidence-backed skips), every branch adversarially verified (one bounce cycle, zero silent failures).
- Lexer deletions: the unraisable-but-exported `LexerError` + its dead 25-line renderer; the dead `normalize_heredoc_delimiter` twin; the module-level `tokenize_with_heredocs` shadow entry that skipped the post-lex pipeline; five dead `TokenStream` methods with their fake "quote tracking" comments; `matches_any_keyword`/`KeywordGuard` (cookbook rewritten around the helpers production actually uses); the never-consulted `case_sensitive` knob end-to-end (NFC normalization retained); the never-read `position_tracker` threading (four call sites - one more than the appraisal found); `pure_helpers.is_inside_expansion` + `find_balanced_parentheses` (a grep for the name now lands only on the live utils implementation - this twin had outlived two prior review flags).
- RD-parser deletions: the `ErrorSeverity`/`expected`/`severity`/`context_tokens`/`CONTROL_KEYWORDS` dead-error-machinery cluster; the unreachable-and-broken `_candidate_split_element` + write-only `inline_tail`; `parser.py`'s write-only `config` alias and argument-discarding `ctx=` parameter; the vestigial multi-token function-name plumbing.
- Two Step-0 currency catches recorded: a config test read the "dead" alias (repointed to its real home) and the position-tracker threading had a fourth site. The census-unreachable backward position-setter branch is now a fail-loud forward-only RuntimeError guard in the module's convention.
- README statistics refreshed for the phase: 16,447 collected tests, 69,106 production lines (255 files) - about 1,400 production lines and 60 dead tests lighter than v0.692.0 with zero behavior change (six full gates, differential corpora, and A/B batteries byte-identical throughout; the one sanctioned fix was v0.696.0's signal-name rendering).

## 0.697.0 (2026-07-11) - Reappraisal-19 Phase D slot 5: test-infrastructure archaeology removed
- Phase D6 of the reappraisal-#19 dead/duplicate-code campaign: net -468 lines of test-infrastructure dead surface (19 files).
- **run_tests.py loses the obsolete subshell `-s` phase**: the constraint it served has been dead since v0.195.0 (the project's own docs said so); subshell tests fold into Phase 1, the `--subshells-only`/`--no-subshells` flags and the lying "need capture disabled" docstring go with it (compare-bash renumbers to Phase 2). Proven standalone (88 passed normal capture, 84 under xdist with the 4 serial-marked correctly excluded) and by the full gates on this and the four prior releases.
- Root `conftest.py` stripped to its load-bearing env pins: the shadowed `shell` fixture that re-taught the retired wait-based teardown, the `_reap_children` twin, and the zero-user `visitor_xfail` machinery are gone. `tests/conftest.py` loses ~150 lines of isolation-era archaeology (MockStdout/MockStderr, `isolated_subprocess_env`, `--strict-isolation`, `--run-slow`, dead `flaky`/`isolated` markers); marker registration consolidates to pytest.ini as the single source (the missing `serial` row added).
- Conformance framework loses its dead half (`is_posix_required`, `get_posix_test_commands`, `save_results`, `get_results_summary`, `_is_bash_specific`/`assert_bash_specific` - the heuristic keyed on constructs psh fully supports); the legacy PTY framework is pruned (4 dead members, vestigial param, bare excepts) rather than ported. Repo-root junk file `test_` removed; MANIFEST.in phantom references pruned.
- Process: dev-implemented from the campaign brief; adversarial verification BOUNCED the first pass (five stale doc references to the deleted surfaces survived in tests/TEST_FRAMEWORK_IMPROVEMENTS.md, including a documented command that would hard-fail) - fixed, re-verified, all repoints tree-wide clean. Collection count unchanged (16,508 -> 16,496 across the whole phase reflects deleted dead tests, not lost coverage).

## 0.696.0 (2026-07-11) - Reappraisal-19 Phase D slot 4: dead surface out of visitor, ast_nodes, utils
- Phase D5 of the reappraisal-#19 dead/duplicate-code campaign: net -274 lines across `psh/visitor`, `psh/ast_nodes`, `psh/utils`.
- **One sanctioned behavior fix, red-on-base proven**: the `signals` builtin now renders EVERY signal by name via the module's real single source of truth (`signal_number_to_name`) - the old hand-rolled 9-entry `SIGNAL_NAMES` dict left anything outside it as `Signal-N` (SIGUSR1 showed as `Signal-30`, SIGWINCH as `Signal-28`). Dict deleted; pin test written first and demonstrated failing on base.
- Deleted: the write-only analysis-visitor state cluster (validator `in_loop`/`in_function`/`variable_names` + the docstring promising a break/continue check that could never fire; security `in_function`/`function_stack`; linter `_in_function`/`_in_subshell`; metrics `in_command_substitution` + three dead CodeMetrics counters) - all four analysis modes proven byte-identical (stdout+stderr+rc) before/after, re-verified at the integration tip; the SignalRegistry report/validate apparatus trimmed to what the `signals` builtin consumes; three dead array-node derived properties (docstrings named consumers that no longer exist; `element_types` kept - it is live); `StatementList.and_or_lists`; the unreachable `_expansion_literal_text` arms; vestigial typing re-exports and `if False:` import guard.
- Also: `SecurityIssue` becomes a dataclass like its siblings; `visitor/__init__.py` `__all__` gains its four missing public names; `ast_debug` unknown-format now raises instead of silently accepting (flag path validates earlier; verified nothing depended on the silence).
- Process: dev-implemented from the campaign brief; 4-agent adversarial verification PASS with zero blockers; integrator re-ran the four-mode byte-identity proof at the final merged tip (all IDENTICAL incl. exit codes) and ratified two declared riders (dead `get_detailed_summary` deletion; the `__all__` completion).

## 0.695.0 (2026-07-11) - Reappraisal-19 Phase D slot 3: dead surface out of builtins, io_redirect, interactive, scripting
- Phase D4 of the reappraisal-#19 dead/duplicate-code campaign: net -93 lines across `psh/builtins`, `psh/io_redirect`, `psh/interactive`, `psh/scripting` (30 files).
- Deleted: `parse_flags`' genuinely-no-op `if consumed_value: continue` (re-verified post-v0.690) plus orphaned assignments; the `disown -h` phantom `job.no_hup` write (`-h` still accepted for compatibility; help text now says so honestly; huponexit recorded in `docs/missing_features.md`); mock-defensive `hasattr`/`try-except` guards in navigation, directory_stack, print and signal_handling production code (zero test conversions needed - no test relied on them); echo's leftover `--debug-exec` scaffolding and double alias layer; `registry.py` dead dunders; `SHOPT_OPTIONS` vestigial identity dict; cd's dead tilde branch behind an always-false hasattr; io_redirect's never-passed `protected`/`check_noclobber`/`redirect=None` parameter chains; the interactive dead-accessor cluster (three HistoryExpander accessors, `_find_word_start` alias, `get_history()`, REPL `prompt_manager` wiring); `FileInput.loaded` + the always-equal line-counter twin ($LINENO verified preserved); `command_accumulator._need_more` (inlined at its 5 sites).
- Structural: `ScriptComponent` ABC dropped (zero abstract methods); `ScriptManager` widened to the real API (`execute_as_main`, `validate_script_file`) with all 7 bypassing call sites routed through the facade; `StringInput`'s display-label sentinel replaced by an explicit `split_lines` parameter.
- Process: dev-implemented from the campaign brief; 4-agent adversarial verification PASS with zero blockers (54-case A/B battery byte-identical vs main except the two sanctioned surfaces); integrator fixups repoint three stale SHOPT_OPTIONS doc mentions and soften the facade docstring.

## 0.694.0 (2026-07-11) - Reappraisal-19 Phase D slot 2: ghost vocabulary and shelfware out of the combinator parser
- Phase D2 of the reappraisal-#19 dead/duplicate-code campaign: net -1,013 lines out of `psh/parser/combinators/` and `psh/parser/visualization/` - the largest single sweep of the phase.
- **Ghost token vocabulary deleted**: parsers for six TokenType names that do not exist (`AND_IF`, `OR_IF`, `EQUALS`, `GLOB`, `ASSIGNMENT`, `HEREDOC_DELIMITER`) plus three `WHITESPACE` checks - none could ever match; the and-or grammar arms were rewired to the real token parsers with the parser differential corpus proven byte-identical before/after (205 cases at three SHAs). The one sanctioned addition: `token(name)` now validates the name against `TokenType.__members__` at construction, so a future ghost fails at import time instead of silently never matching.
- **Shelfware combinator algebra deleted** (appraisal H18): `between`, `literal`, `lazy`, `try_parse`, `separated_by`, `with_error_context`, `ForwardParser`, `sequence`, `skip` had zero live-grammar call sites - the real grammar is hand-rolled closures over `token`/`keyword`/`many`/`many1`/`optional`/`fail_with`/`or_else`. `docs/guides/combinator_parser_guide.md` rewritten to describe the actual closure style (it previously taught that pipelines chain via `separated_by()` - fiction since the module landed).
- Also deleted: 14 dead keyword parsers + twin do/then separators (implemented twice, differently, used never), three parallel test-only APIs in tokens.py (static factories, module-level convenience functions, predicates), `ExpansionParsers` stripped to its one live method (`build_word_from_token`), the no-op `configure()`, ~16 unused package imports, and dead visualization surface (write-only `node_ids`, v0.120 `arg_types`/`quote_types` fossils, zero-caller sexp wrappers).
- Process: dev-implemented from the campaign brief; 4-agent adversarial verification PASS with zero blockers; integrator fixups for three claim-bearing doc lines that still named deleted combinators.

## 0.693.0 (2026-07-11) - Reappraisal-19 Phase D slot 1: dead surface out of core, executor, expansion, arithmetic
- First shipped slot of the reappraisal-#19 dead/duplicate-code campaign (`docs/reviews/reappraisal_19_campaign_briefs_2026-07-11.md`, Phase D3): net -174 lines of dead or vestigial surface deleted across `psh/core`, `psh/executor`, `psh/expansion`, and the arithmetic engine, with zero behavior change (bash-probe verified where the line was blurry).
- Deleted: the five dead typed accessors on ShellOptions (and their false "hottest internal reads" rationale), `ScopeManager.has_variable`, the vestigial `VariableScope.parent` field, the `Process.stopped`/`.completed` back-compat shims (their one live reader repointed to `proc.state is ProcessState.COMPLETED` - pure state-test shims, behavior-identical), `PipelineContext`'s unread `job_manager` parameter and write-only `job` attribute, ~10 unconsumed `executor/__init__` re-imports, the verbatim-duplicated exec-OSError formatting block, `BraceExpander._contains_expandable_dollar`, production-dead `match_extglob` (+ export + its tests), dead arithmetic back-compat re-exports, and assorted dead checks and aliases.
- Fixed in place: eight stale `adopt()` -> `clone_for_child` docstring pointers, arithmetic public-node export drift, the `pwd` module-shadowing local, nine arithmetic function-local imports hoisted to module scope (import-cycle-free), and the cryptic `"'_' * 2"` respelled as `'__'` with an evidence-based comment - a 14-case bash-vs-psh probe battery (archived under the campaign ledgers) proved the exclusion is NOT inert (the cycle-error token flips without it), so it stays, now documented.
- Process: implemented by a dev agent from the campaign brief; adversarially verified by a 4-agent pass (deletions-only diff audit, symbol-resurrection hunt, ledger cross-check, claim re-probe) - PASS with zero blockers, probe battery independently replayed at the final SHA.

## 0.692.0 (2026-07-11) - Jobs depth: jobs -n/-x land, non-interactive job state is real, and notifications follow bash's modes
- **`jobs -n` and `jobs -x` are implemented** (the synopsis grows to bash's full `jobs [-lnprs] [jobspec ...] or jobs -x command [args]`): `-n` shows only jobs whose status changed since last notification, via a centralized notified-state lifecycle on `Job.update_state` (any listing marks a job notified; a real state transition clears it) — the long-deferred notification predicate now lives in ONE place; `-x` substitutes whole-word jobspecs (`%1`, `%+`, `%?str`) with process-group ids and runs the command through normal resolution (functions included).
- **Non-interactive `jobs`/`fg` see real job state**: a new per-job poll (`refresh_job_states`, waiting on each tracked job's OWN process group) replaces reliance on the interactive-only reaper — designed so it structurally cannot steal command-substitution or process-substitution children (they live in the shell's process group, invisible to a per-job wait; the v0.689.0 wait machinery is untouched and byte-identical). A job's state is recomputed only when a real pid is reaped. Stop-reflection is monitor-gated like bash (an externally-stopped job lists as Running without `set -m`).
- **Completed-job listing now follows bash's modes** (found by adversarial verification — the initial claim that bash never lists completed jobs held only for `-c`): in script-file and stdin modes, `jobs` lists a finished background job once (`[1]+ Exit 1 false` / `Done`), then reaps it (remembered statuses preserved, so a later `wait $p` still returns the real code); in `-c` mode and interactively the listing is empty (bash reaps eagerly / via the prompt notice). The test suite's mode-blindness — every job pin previously ran via `-c` — is fixed structurally with live-bash pins in all three non-interactive read paths.
- Also: `set -b`'s Done notice no longer prints in non-interactive shells (psh over-notified; bash reserves it for interactive prompts — the previous test pinned the wrong behavior and was flipped with probe evidence); `fg`/`bg` accept `--`; a stopped job foregrounded with monitor on but no tty now resumes to completion with the job's real exit code (was 128+SIGSTOP); background job command text no longer carries a trailing space. Deferred with full probe writeups: bash's `-c`+`set -m` async boundary notice (a read-loop artifact; timing-flaky to pin) and its downstream `jobs -n` stop-reshow cell.
- 30 tests + 7 goldens + 9 mode-matrix pins (red-on-base demonstrated throughout); adversarially verified through two rounds — the v0.689.0 wait internals confirmed byte-identical, six mutation arms killed (including a waitpid(-1) steal), the mode matrix diffed cell-by-cell against a verifier-independent bash oracle, and one bounce (the completed-listing claim) fixed mode-aware before ship. Suite grows to 15,066 gate-passed; compare-bash 1,340; goldens 1,363; collected 16,508; mypy 255.

## 0.691.0 (2026-07-11) - Array-element escapes: residual backslashes and assoc value escapes match bash
- **`declare -a arr=(a\\b)` no longer collapses to a scalar.** The array-initializer lookup key was the raw flat text while declare looked up by the escape-collapsed argv — any element whose collapse leaves a residual backslash (e.g. `a\\b` → `a\b`) missed the lookup and the whole `(...)` became element[0]. The key is now computed by the new `array_init_argv_key()` (psh/parser/array_flat_text.py), which delegates to the expansion path's own unquoted-escape transform, so key == argv by construction and escape-free arrays are byte-identical. This deliberately reopens the wave-3 (v0.687.0) "serializer-unfixable" verdict: that held only within the old model, where the flat text served double duty as both literal-part source and lookup key; keying on the transformed argv breaks the double role. (An adversarial re-derivation, recorded in the helper's docstring.)
- **Assoc element VALUE escapes match bash**: `declare -A a=([k]=a\$b)` now stores `a$b` (and `\"`, `\'`, `` \` `` likewise) — the operator-debris word recognizer stranded a backslash before `$`/quotes; it now consumes `\<char>` as a literal pair. Not array-specific: `echo [k]=a\$b` is fixed too. Subscript tokenization is untouched, so `[$k]=v` / `["a b"]=v` keep expanding (regression-pinned, mutation-proven load-bearing). A side effect is a net improvement on `[k]=a\)` (the escaped `)` no longer silently closes the initializer; psh now rejects it like bash, modulo a ledgered rc 2-vs-1 syntax-error-code class).
- 27-cell truth table (6 divergences → 0); frozen lexer corpus verified unchanged (zero rows — independently confirmed by the verifier over all 957 inputs); both parsers byte-matched via shared-helper construction plus parity pins. 17 unit pins + 15 goldens (red-on-base demonstrated); adversarially verified before ship: 96 independent file-oracle cases all bash-matched, 17 fixed / 0 regressions / 43 byte-identical on base A/B, all mutation arms killed. Suite grows to 15,020 gate-passed; compare-bash 1,333; goldens 1,356; collected 16,456; mypy 255.

## 0.690.0 (2026-07-11) - Runtime errors carry bash's location prefix: `psh: line N: origin: message`
- **Non-interactive runtime diagnostics are now location-prefixed like bash's** (`<$0>: line N: <origin>: <msg>`): all builtin errors (223 `Builtin.error` sites), external command-not-found and exec failures (including forked children, fd-level), `set -u` unbound-variable (primary path AND the case-subject/for-words boundary fallback), the readonly-assignment class (executor and arithmetic evaluator), expansion errors (`${x:?}`, bad substitution, invalid indirection, and family), the trap set-path invalid-signal diagnostic, the nameref-cycle and command-substitution null-byte warnings, and getopts' special unnamed form. `<$0>` is psh's invocation analogue ("psh" for `-c`/stdin, the script path in script mode); `line N` tracks $LINENO and is dropped in interactive shells, exactly as bash does. Bash's dual-line shape is preserved: the runtime line is prefixed while usage lines stay bare (`trap -x` prints both), and pure usage errors are never prefixed. The SHELLOPTS env-import error uses bash's startup sentinel `line 0` with argv0 (probed: bash uses argv0 there even in script mode).
- Truth-tabled across a full mode×class matrix (multi-line `-c`, script, stdin, sourced, interactive PTY; line numbers verified through command substitution, pipelines, eval, and heredocs). Deliberately excluded and queued: redirection-error line numbers (io_redirect subsystem), message-body divergences, and psh's rich parse-error format (a ruled educational divergence, untouched). Ledgered name-only divergences: bash's `environment:` quirk for `-c`-defined functions and sourced-file naming (no BASH_SOURCE stack).
- 45 new pins (unit + exact-live-bash conformance, red-on-base demonstrated) and 31 collateral tests read-verified against bash and updated (none loosened). Adversarially verified through three verification rounds before ship: an independent mode×class re-probe, six mutation arms (including over-application of the prefix to usage lines), a real-PTY interactive check, and a transcript-audited re-probe of every deliberately-bare diagnostic site — the rounds caught two missed sites, a falsified "verified-bare" ledger disposition (corrected with archived bash-vs-psh transcripts for every such row), and a mutation-proven pin gap, all closed and pinned. Suite grows to 14,988 gate-passed; compare-bash 1,318; goldens 1,341; collected 16,409; mypy 255.

## 0.689.0 (2026-07-10) - Wait robustness: the ECHILD reap-timing race is closed, and `wait -n` remembers statuses
- **Closes the reap-timing window behind the one flaky golden** (r18t1, seen once under full xdist load, 2026-07-10): when a bare `wait`'s process-group `waitpid(-pgid)` returned ECHILD, psh assumed the job had been reaped elsewhere and marked it orphaned-completed — leaking any still-alive child the group wait missed, so a later `wait $p` reaped the child's real status (rc 5) instead of bash's rc 127 for an already-collected pid. The ECHILD branch now reclaims each still-running process by its specific pid first (`waitpid(pid, WUNTRACED)`, recording stopped children as stopped, never completed); per-pid ECHILD still falls through, preserving the interactive SIGCHLD-reaper case. The race was made deterministically reproducible via an inert, env-gated test seam (`PSH_TEST_FORCE_GROUPWAIT_ECHILD`, default-off; verified benign suite-wide even when armed) — the fix is pinned red-on-base through it, including a multi-process pipeline reclaim pin and a stopped-child pin whose base behavior was a hang.
- **`wait -n` now remembers the reaped job's status** like bash: `( exit 6 ) & p=$!; wait -n; wait $p` gives 6 then 6 (psh previously forgot, giving 127), while bare `wait` still clears remembered statuses (symmetric cases pinned; all bash-verified against an 11-case battery).
- Adversarially verified before ship: the leak mechanism independently reproduced on base, the fix 10/10 deterministic through the seam, five mutation arms (including first-proc-only reclaim and stopped-as-completed) each killed by a pin, and the armed seam proven inert across the whole job-control suite. Suite grows to 14,942 gate-passed; compare-bash 1,318; goldens 1,341; collected 16,363; mypy 255.

## 0.688.0 (2026-07-10) - Locale Stage 4: LC_*/LANG changes take effect mid-session, and PEP 538's phantom LC_CTYPE is stripped
- **Locale variables are now reactive**, not startup-only: assigning, unsetting, or exporting `LC_ALL`, `LC_CTYPE`, `LC_COLLATE`, or `LANG` mid-session re-derives the shell's locale behavior the way bash does — pattern/`[[:alpha:]]` classification, `${v^^}`-style case mapping, and `[[ <\` collation all follow, with bash's precedence (`LC_ALL` > category > `LANG`), reversion on unset, and bash's special-case that even an *unexported* assignment to these names changes the shell's own behavior. `LC_ALL=C cmd` one-command overlays react on entry and revert on exit via the existing temporary-env machinery; subshell and command-substitution locale changes stay isolated from the parent. Implemented as `LocaleService.reinit` driven by the reactive-specials observer (the same seam as OPTIND/POSIXLY_CORRECT), gated to the four names; libc's setlocale is still touched only for non-C locales.
- **PEP 538 coercion no longer leaks into the shell.** In bare-C environments CPython rewrites `LC_CTYPE` to `C.UTF-8` before psh sees it; psh previously imported that phantom as a real exported variable — so `$LC_CTYPE` was wrongly non-empty, `é` classified as alphabetic where bash's C locale says otherwise, and children inherited an `LC_CTYPE` bash would never pass. psh now detects the coercion at startup (UTF-8 mode plus a coerced value, distinguishing it from a genuinely user-set `C.UTF-8`) and strips it, matching bash on all three surfaces. One corner is documented as a known approximation (a user-set `C.UTF-8` combined with an explicit `PYTHONUTF8=1` runtime knob would also be stripped — no realistic shell path reaches it).
- 19 conformance + 12 subprocess + 5 unit pins (demonstrated red-on-base) + 6 goldens chosen for platform stability (identical under macOS libc and glibc, keeping the Linux nightly safe). Adversarially verified before ship: independent 28-case env-controlled battery all-match, five mutation arms killed by pins, discriminator matrix reproduced, no over-stripping in default environments. Suite grows to 14,932 gate-passed; compare-bash 1,313; goldens 1,336; collected 16,348; mypy 255.

## 0.687.0 (2026-07-10) - Literal patterns: bash-correct brackets, declare array escapes, double-quote backslash runs, escaped-$ braces
- **Unclosed `[` no longer eats the line.** The lexer's glob-bracket scanner followed a psh-only rule where an unclosed `[` swallowed whitespace and terminators to end of input — so `v=[` consumed the rest of the line and `case`-pattern words could absorb `)`. psh now follows bash's rule: in argument context `[` never affects word boundaries at lex time (`printf "<%s>" x[y z]w` → `<x[y><z]w>`, matching bash), an unclosed `[` is a literal character, and `v=[` assigns a literal bracket. Assignment-subscript words (`a[0 + 1]=v`) are a separate lexer path and keep their bash-matched behavior. The bracket tracker is also escape-aware, so `\[` in a case pattern no longer counts as an open bracket. Exactly 2 frozen-corpus rows changed, each individually re-verified against bash.
- **`declare -a arr=(a\$b c)` no longer collapses.** A backslash anywhere in a declare array-initializer body made the whole `(...)` become element[0]; the array-init lookup key is now built by escape-processing unquoted literal parts (new shared `psh/parser/array_flat_text.py`, used by BOTH parsers), so the key equals the escape-processed argv. The long-xfailed declare -p round-trip test now genuinely passes. (Two narrower element-escape residuals remain, ledgered as a queue task — both pre-existing, one improved.)
- **Double-quoted backslash runs match bash.** The expansion layer was double-decoding backslashes the lexer had already decoded (`"a\\\b"` printed `a\b` instead of bash's `a\\b`); the double-quote escape pass now only strips the one thing the lexer leaves behind (`\$`), fixing arguments, assignment values, here-strings, and `[[ ]]` operands alike. Heredoc bodies were already correct and are untouched.
- **`\${a,b}` expands like bash**: an escaped `$` before `{` no longer suppresses brace expansion — `echo \${a,b}` gives `$a $b` (escape-parity-aware detection).
- The truth-table re-probe that scoped this release also found most historical case-pattern metachar divergences and the composite-`$'\n'` divergence already fixed by the v0.682 WordToken work (regression-pinned; the stale queue items were closed as dissolved).
- 21 pins (red-on-base demonstrated) + 23 goldens; adversarially verified before ship: two independent three-way differentials (base/branch/bash) with zero regressions on valid inputs, corpus completeness proven over all 957 rows, per-fix and partial mutation testing all killed by pins, combinator parity byte-identical. One exposed pre-existing parser gap on malformed input (unclosed `NAME[` at command start now runs where bash syntax-errors) ledgered as a queue task. Suite grows to 14,890 gate-passed; compare-bash 1,307; goldens 1,330; collected 16,300; mypy 255.

## 0.686.0 (2026-07-10) - Core-state polish: hashall rename, SHELLOPTS/BASHOPTS enumeration, posix readonly exit class
- The `set -o` option formerly named `hashcmds` is renamed to bash's canonical **`hashall`** on every surface (`set -o`/`set +o` listings, `shopt -o`/`-po`, `$SHELLOPTS`, `set -h`, help); `hashcmds` is now REJECTED exactly as bash rejects it, and a stale `hashcmds` in an imported `SHELLOPTS` warns like any unknown name. The now-dead env-import alias table was removed (its emptied husk briefly tripped the clone-completeness meta-guard — the guard working as designed).
- No-arg `set` and `declare -p` now enumerate the computed specials **SHELLOPTS and BASHOPTS** with live values via the one shared enumeration method (presence and line format match bash; the values legitimately differ where psh's option sets differ). The dynamic RANDOM/SECONDS family stays out of no-arg listings as a documented deliberate divergence — bash's own rendering there is reference-state-dependent (an unreferenced `SECONDS` prints differently than one that has been read).
- In posix mode under `-c`, a plain assignment to a readonly variable (`SHELLOPTS=x` style) now exits with **rc 127**, matching bash. This deliberately reopens the v0.677 decision that had pinned rc 1 ("an artifact not reproduced"): psh already reproduces bash's 127 for the sibling fatal expansion errors (`set -u`, `${x:?}`) in `-c` mode, so the readonly case was the one family member left inconsistent. Script-file and stdin modes keep rc 1; the readonly/export builtin forms and prefix-assignment forms keep rc 1 via the v0.673 SpecialBuiltinUsageError machinery (all branches pinned both ways).
- 12 new goldens + new/updated pins (demonstrated red-on-base); adversarially verified before ship (independent re-probe of every surface incl. a 36-cell posix/rc matrix, five mutation arms all killed by pins, bounce-commit purity diff, red-on-base reproduction — all clean). Suite grows to 14,856 gate-passed; compare-bash 1,284; goldens 1,307; collected 16,244; mypy 254.
...
- Truth-tabled against bash 5.2 before fixing; the cluster's other two seeds dissolved: `return`'s diagnostic matrix (non-numeric, out-of-range, outside-function, sourced) already matches bash, and psh's rich parse-error format (caret/context/suggestions) is recorded as a deliberate educational divergence rather than a bug. Two systemic findings ledgered as future tasks: bash's `prog: line N:` prefix on builtin runtime errors, and non-interactive job-notification depth (`jobs -n`/`-x`).
- 21 new pins (unit + conformance, demonstrated red-on-base) + 6 goldens; adversarially verified before ship (independent 36-case bash re-probe, full and partial mutation testing, red-on-base evidence reproduction, collateral sweep — all clean). Suite grows to 14,821 gate-passed; compare-bash 1,268; goldens 1,291; collected 16,193; mypy 254.

## 0.683.0 (2026-07-10) - R3 fix-forward: two WordToken regressions fixed, and a correction to the v0.682.0 record
- **Fixes two regressions that shipped in v0.682.0** (both introduced by the word-fusion change, both parsers): (1) a composite-headed word containing `[..]=` or `=(` — `"q"a[0]=v`, `a[0]$x=y`, `"q"a=(1 2)`, `${v}a[0]=v`, `p$'q'[0]=v` and family — was misparsed as an array assignment, silently succeeding (rc 0) or creating a spurious array where bash and v0.681.0 run it as a command (rc 127) or reject it (rc 2); the array-assignment classifier now requires the `name[subscript]op` / `name=(` head to lie entirely within an unquoted leading literal part with a valid identifier. (2) A same-line `alias` definition whose value contains a simple `$var` lost the `$` in the absorbed value (`alias e2="echo pre$v"; e2` printed `prev`); alias values are now reconstructed source-faithfully from token parts.
- **CORRECTION to the v0.682.0 changelog entry below**: that entry claimed the recursive-descent parser was verified byte-identical with "13 combinator divergences, all improvements." That claim was FALSE — the release's differential evidence run was invalid (stale tree / import-resolution gotcha), and re-running the same harness at the shipped commit shows 37 divergences, including the 5 recursive-descent regression shapes fixed here. v0.682.0 was also merged without the campaign's adversarial verification step; the post-hoc verification that found these defects is now the record. The differential harness itself was hardened (stderr first-line comparison; per-tree import pinning).
- Pin gaps closed: the load-bearing fusion-after-keyword-normalization ordering, the arithmetic-context fusion gate (semantic test), the array-init regression golden (replaced a vacuous single-token pin with a genuinely-fused composite element), and the full improvement surface (22 case/`[[ ]]` + 4 bracket-word combinator improvements and 3 recursive-descent `[[` operand improvements, all bash-verified, pinned red-on-base).
- One pre-existing divergence ledgered, not chased: bare `]]` gives rc 127 in psh (both parsers, pre-existing on the RD side) vs bash's rc 2 syntax error.
- Independently verified before shipping (the process v0.682.0 skipped): all fix shapes red-on-shipped and bash-matched in both parsers, three mutation arms killed by their pins (6/10/6), differential re-runs reproduced exactly (27 improvements-only vs v0.681.0; exactly the fix surface vs v0.682.0), production diff surgically limited to three files. Suite grows to 14,794 gate-passed (+96 pins); compare-bash 1,262, goldens 1,285 (one replaced), mypy 254.

## 0.682.0 (2026-07-10) - Lexer R3 (WordToken): one structured WORD token per shell word
- Final release of the lexer Phases 3-6 campaign (R3 of 3) and its only corpus re-freeze. The lexer now emits ONE `WORD` token per shell word: `word_fusion.fuse_words` (run in `_post_lex`, after keyword normalization — the parser's old `peek_composite_sequence` relocated) composites a maximal run of adjacent word-like tokens into a single WORD carrying the run's `parts`, so the parser sees one token per shell word and never re-assembles a composite. Fusion is suppressed inside `(( ))` / C-style `for` headers (those interiors are consumed by `collect_arithmetic_expression`, not composited).
- Retires the compatibility token machinery: `RichToken` (the base `Token` already carries `parts`), `TokenStream.peek_composite_sequence`, `WordBuilder.build_composite_word`, the combinator's `_group_adjacent_tokens`, and the `TokenType.COMPOSITE` (never emitted) and `TokenType.PARAM_EXPANSION` (emit-dead — the lexer emits `VARIABLE` for every `${...}`) enum values, across every acceptance list and dispatch in both parsers. The consumers that build Word AST nodes (both parsers' `build_word_from_token`, array-init/element parsing, `[[ ]]` test/regex operands, the alias same-stream overlay) migrate to read the fused word's parts.
- **Corpus re-freeze** (the campaign's only one): a committed generator (`tools/regen_lexer_corpus.py`) transforms the OLD frozen `lexer_stream_corpus.json` into the NEW one by an INDEPENDENT mechanical adjacency-collapse; its `--verify` confirms the new lexer byte-matches that transform on all 957 rows (524 changed by fusion, 0 disagreements — proving fusion == the mechanical collapse, no hidden boundary moves). The 2 rows a naive estimate over-predicted are `(( a["x"] ))` / `(( a[$'q'] ))`, whose only collapsible run is inside `(( ))` and is correctly arith-gated.
- Behavior-preserving by construction: the recursive-descent parser is BYTE-IDENTICAL base-vs-branch across a ~1,050-fragment execution differential over both parsers; the combinator is byte-identical too except that fusion INCIDENTALLY fixed 13 spurious combinator parse-errors on composite `case`/`[[ ]]` patterns (now bash-correct, matching RD — pinned red-on-base/green-on-branch). Two real regressions the suite/differential caught were fixed and pinned: `declare -A m=([k]=$'a\tb')` composite values (array-init flat text must carry no leftover escape so the declaration builtin's post-escape lookup matches), and the combinator's for/select `items` string view (use the word's `display_text`, matching RD).
- Suite grows to 14,698 gate-passed; compare-bash 1,262 (+1 regression-pin golden); goldens 1,285; mypy 254 files; collected 16,064.

## 0.681.0 (2026-07-10) - Lexer R2: frozen source-faithful tokens, one SourceMap, and dispatch order you can read
- Second release of the lexer Phases 3-6 campaign (R2 of 3), two pure-preservation refactors. **Phase B**: the recognizer registry's numeric-priority mechanism (per-class `priority` properties + sort-on-demand) is gone — recognizers dispatch in one explicit registration order declared in a single place, and the dead `WhitespaceRecognizer` (provably unreachable: the main loop skips whitespace before dispatch) is deleted. **Phase C**: `Token` and `RichToken` are `frozen=True`; a `span` property exposes a `SourceSpan`; ONE `SourceMap` service (offset → line/column + line text, in `position.py`) backs both `PositionTracker` and the parser's error context; the previously-invisible dynamic attributes `heredoc_key` and `array_init` become declared fields with None-sentinel semantics; `KeywordNormalizer` is non-mutating (rebuilds classified tokens); the combinator's parse-time `is_keyword` mutation becomes a pure predicate; the inert `strict` lexer parameter is retired. Stored `line`/`column` are deliberately KEPT (the plan's "derived" migration was ruled off — zero payoff for real error-path risk).
- Verified adversarially by a 5-lens fan-out plus a bounce cycle: two independent differential harnesses (1,092- and 1,527-fragment, full-field including exception class+message) byte-identical base-vs-branch; a 110-case error-message battery byte-identical (earning SourceMap its place in the error path); the dev's canonicalizing comparator audited sound; dispatch-order equivalence proven against the old priorities; frozen-ness stress-run over alias splice, heredoc collection, and both parsers with zero latent mutation crashes; corpus byte-frozen (sha256 unchanged).
- The bounce: mutation testing found the paths Phase C rewrote had NO diagnostic pins — losing `line`/`column` in the keyword rebuild changed user-visible parse-error output on 8 shapes while the entire 14,655-test gate stayed green. Fixed with 11 new pins (exact line/column + caret-block diagnostics for keyword-anchored errors, a dedicated `array_init` consumer pin, a `normalize()` non-mutation pin), each demonstrated red-under-mutation and independently re-confirmed (8/2/1 kills).
- Suite grows to 14,666 gate-passed; compare-bash 1,261 and goldens 1,284 unchanged (pure preservation, zero red-on-base pins). mypy count 254 → 253 files (one module deleted).

## 0.680.0 (2026-07-10) - Lexer R1: the command-position booleans become an explicit LexicalState with one transition function
- First release of the lexer Phases 3-6 campaign (R1 of 3). The lexer's cross-token state — four ad-hoc booleans (`command_position`, `case_depth`, `case_expecting_in`, `in_case_pattern`) updated inline by a 116-line method — is now an explicit `LexicalState` (`LexicalRole` enum for the command axis, case phases readable as a `CasePhase` view) advanced by ONE pure transition function, `advance_lexical_state()` in `command_position.py`. `LexerContext` remains as a compatibility alias with the pinned kwargs construction API; `command_position`/`in_case_pattern` are derived read properties, so every recognizer is untouched and the token stream is byte-identical.
- Deliberately NOT a cross-machine unification: the campaign plan's "three machines → one transition function" was found (by the mandatory internals scout) to contradict the reviewed v0.649 decision that per-stage transition shapes are irreducible — sharing lives at the vocabulary level, which is already test-locked. The transition function is lexer-stage-only and says so in its docstring; the KeywordNormalizer and cmdsub scanner keep their own machines.
- The refactor makes the case state machine honest: the scout proved `(case_expecting_in, in_case_pattern) = (True, True)` is REACHABLE on malformed input (`case x ;;`), so the bits are independent and now pinned that way — a collapsed single-phase enum would have changed behavior.
- Pure preservation, verified adversarially by a 5-lens fan-out: 29,176 differential tokenizations (3,647 adversarial fragments × 4 modes × 2 entry points) byte-identical base-vs-branch; 400 malformed + 1,000 fuzz fragments identical (the `(T,T)` state reached 206 times); 7/7 semantic mutations of the transition function killed by named pins; the frozen 957-row lexer corpus byte-identical (sha256 unchanged); ledger claims spot-audited 5/5. A 58-row psh-vs-bash truth table over command-position constructs was all-green on base and branch — zero behavior change, zero red-on-base pins.
- Suite grows to 14,638 gate-passed (+53 new state-machine/case-subject pins); compare-bash 1,261 and goldens 1,284 unchanged (no behavior to pin).

## 0.679.0 (2026-07-09) - `VAR=x cmd` is a real temporary environment now — name lookup sees it, `set`/`export -p`/`declare -p` don't
- Before, a `VAR=x cmd` prefix over a builtin or external bound `VAR` as a real exported shell variable for the command's duration, so it LEAKED into whole-table enumerations — `FOO=bar set`, `FOO=bar export -p`, and `FOO=bar declare -p` all listed `FOO`, and an override (`export E=orig; E=temp export -p`) showed the temporary value. bash keeps such a prefix in a SEPARATE temporary environment that name lookup consults but enumerations skip. Now psh does too: a hidden `ScopeManager.command_temp_env` layer that `$VAR` / `${VAR}` / `declare -p VAR` / `${VAR@a}` / the command's own process environment read, while `set` / `export -p` / `declare -p` (no name) — which scan the scope stack — skip it automatically. All three enumeration divergences close.
- Because the binding is not a shell variable, it correctly does not inherit the shadowed variable's attributes (`declare -i n=5; n=abc cmd` gives the command a plain `abc`, not `0`), and a scalar prefix over an array shadows non-destructively (fixing a base bug where element 0 was mutated). Naming the binding with `export`/`readonly`/`declare -x` PROMOTES it to a real persistent variable carrying the temporary value (which wins over any real same-name var), matching bash including attribute handling (`declare -ix` carries `-i`, bare `declare -i` does not).
- A prefix over a FUNCTION stays bash's temporary VARIABLE CONTEXT (merged into the function's locals, so enumerations run inside the body DO list it) — psh keeps the two mechanisms distinct, matching bash on both. The `read` builtin now fetches `IFS` via name lookup (so `IFS=: read` sees the prefix) rather than the enumeration path.
- One accepted, documented divergence: a prefix over `eval`/`source`/`.` whose body then runs a whole-table enumerator — bash makes the outer prefix visible to that nested enumeration, psh consistently hides it. This is a deliberate new divergence (psh's consistent hiding is cleaner and the corner is vanishingly rare), documented in the differences guide and pinned as intentional.
- Independently verified: ~110-row differential battery vs bash 5.2.26, all promotion/attribute/function-context/fork cases confirmed, mutation-tested (leaking the enumeration kills the skip-pins; disabling lookup kills 15 pins), 33 pins red-on-base. Suite 14,585 gate-passed, compare-bash 1,261 with 17 new `tempenv_*` goldens (1,284 total).

## 0.678.0 (2026-07-09) - Brace expansion moved to the Word stage, where bash does it — the parse-time toggle scanner is gone
- psh brace-expanded at TOKENIZE time and relied on a same-stream `set ±B` / `shopt -so braceexpand` look-ahead scanner to make `set +B; echo {a,b}` match bash — a documented approximation that got six edge classes wrong. Brace expansion now runs at the WORD stage (a new `WordBraceExpander`, one `Word → List[Word]`), reading the LIVE braceexpand option per-command at execution time, so no scanner is needed. `brace_expansion_tokens.py` (the TokenBraceExpander plus both `_interpret_set_args`/`_interpret_shopt_args` scanners) is deleted.
- All six ledgered approximation classes are now correct vs bash: a toggle in a not-taken branch / uncalled function no longer flips the rest of the parse unit; loop bodies re-read the option per iteration; a function-shadowed `set`/`shopt` doesn't toggle; a `set +B` in a later pipeline segment (a subshell in bash) correctly doesn't; invalid cluster letters that abort the builtin don't toggle; and quoted/dynamic operands (`shopt -so "braceexpand"`) now work at runtime — all verified red-on-base, green-on-tip.
- The Word AST couldn't distinguish `$v` (fusable with an adjacent brace) from `${v}` (not) — both parsed to the same node. Added `VariableExpansion.braced` (with `compare=False, repr=False` so AST-repr corpora and node-equality stay byte-identical); fusion fires only for bare, unquoted, NAME-shaped vars, matching bash (`$v{1,2}`→`A B`, `${v}{1,2}`→`1 2`).
- Rider (same anti-pattern): `parse-tree` tokenized without threading shell options, so `shopt -s extglob; parse-tree '@(a|b)'` printed a parse error instead of the extglob AST the executor builds — now threads `state.options`.
- Deliberate divergence (ledgered): a brace expansion exceeding psh's 100,000-item resource guard now exits 1 (a runtime Word-stage error) rather than the old parse-time syntax class 2 — bash has no such limit; it stays loud-not-silent (stderr message, empty stdout, shell continues) and is now CONSISTENT across all five field-producing contexts (simple/for/select/array-init/redirect), a consistency the adversarial verifier caught and the dev fixed mid-wave.
- Independently verified: ~130-row brace-surface sweep vs bash 5.2.26 with zero regressions, all six classes reproduced red-on-base, the placeholder encoding proven collision-free, both coupling directions mutation-tested. Suite 14,508 gate-passed, compare-bash 1,244 with 13 new brace goldens (1,267 total).

## 0.677.0 (2026-07-09) - Input edges match bash: CRLF isn't a continuation, `read` keeps trailing whitespace only at true EOF, `set -v` stops double-spacing
- `\<CR><LF>` was wrongly treated as a line continuation (psh joined the lines); bash treats the backslash as escaping the CR, so the two commands stay separate. Fixed for stdin and `-c` (a genuine all-CRLF DOS script still normalizes via the existing FileInput CR-strip; a contrived mixed-CRLF file is the one ledgered residual).
- A non-raw `read` hitting a trailing backslash now keeps the last field's trailing whitespace/delimiter ONLY when the backslash lands at RAW EOF (no newline after it); a delimiter-terminated `\<LF>` continuation that then runs into EOF is an ordinary join with no marker — matching bash across single/multi-var, custom IFS, and `read -a` (which previously produced a phantom extra array element). `read -r` is unaffected.
- `set -v` no longer emits a spurious blank line when a trailing continuation flushes into EOF (the verbose echo double-counted a load-bearing reprieve newline).
- The `--validate`-mode EOF-backslash drop rule, previously guarded by one rc-only test that survived mutation, now has a `--format` discriminator that distinguishes stream (drop) from string (keep) and is mutation-verified.
- The `read` fix was caught mid-wave by the adversarial verifier as a regression (the first cut kept the marker whenever the NEXT read hit EOF, breaking the common `while read` over a file whose last line ends in `\`); re-fixed to gate on the INCOMING read status and re-verified against a 14-row Case-B battery with the `read -a` array-length pin. No regressions to the v0.674 EOF stream-vs-string split (25/25 matrix preserved). Suite grows to 14,459 gate-passed, compare-bash phase 1,231 with 8 `read_bseof_*` golden cases (1,254 total).

## 0.676.0 (2026-07-09) - `--posix` and `POSIXLY_CORRECT` finally turn posix mode on the way bash does
- Before, psh's posix mode was reachable only via `set -o posix` mid-session — there was no `--posix` command-line flag and no `POSIXLY_CORRECT` environment support. Now `--posix` and an inherited `POSIXLY_CORRECT` (present at startup with any value, including empty) both enable posix mode before anything runs, wiring straight into the existing option so v0.673's special-builtin exit-on-error fires from a `--posix script.sh` or `POSIXLY_CORRECT=1 script.sh` invocation.
- `POSIXLY_CORRECT` couples to the posix option in BOTH directions, matching bash exactly: assigning it mid-session (any value) turns posix on, `unset`ting it turns posix off, `set -o posix` creates `POSIXLY_CORRECT=y` (unexported, no clobber of a user-set value), and `set +o posix` unsets the variable. The coupling reuses v0.675's `ShellOptions.on_change` observer and startup env-import — no parallel machinery — and threads through fork cloning so subshells track independently of the parent.
- Cross-release interactions verified end-to-end by the adversarial verifier: the flag/env reach v0.673's exit policy identically via all three activation paths (including the suppressible `if`/`&&`/`||`/`!` guard classes), `$SHELLOPTS` (v0.675) lists posix in both its computed value and exported entry once flipped on, and a one-shot `POSIXLY_CORRECT=1 cmd` prefix (v0.669 temporary-env) enables posix for that command only.
- `sh`-name invocation (argv0 basename `sh`) is deliberately out of scope (broader than posix) and left for a separate item; a pathological readonly-`POSIXLY_CORRECT` corner is ledgered (bash force-unbinds it ignoring readonly, psh respects readonly) and proven non-corrupting — the variable→option observer can never fire on a failed write, so posix stays functionally consistent.
- Independently verified: full two-way coupling truth table vs bash 5.2.26, both coupling directions mutation-tested (each direction kills its own pins), red-on-base recounted exactly (27 red / 11 controls + 3 goldens), a vacuous flag→exit regression guard strengthened to genuinely fail on base, and the v0.675 SHELLOPTS-import regression landmine confirmed intact. Suite grows to 14,441 gate-passed (+44), compare-bash phase 1,223 with 5 `posixly_*` golden cases (1,246 total).

## 0.675.0 (2026-07-09) - The option-reflection surfaces exist now: shopt -o, SHELLOPTS, BASHOPTS — one table behind them all
- `shopt -o` was rejected outright and `$SHELLOPTS` was empty. Now: `shopt -o NAME` queries set-o options with bash's rc semantics (0=on, 1=off), `shopt -so`/`-uo` set/unset them (sharing one `apply_set_o_option` engine with SetBuiltin), `shopt -po` prints reusable `set ±o` lines, and the full bash flag grammar holds (clustering, flags-stop-at-first-operand, `-s`+`-u` rc 1, `-s/-u`-wins-over-`-p` with operands, the probe-pinned `shopt -so BADNAME` rc-0 quirk). bash's identities `diff <(set -o) <(shopt -o)` and `diff <(set +o) <(shopt -po)` are both empty in psh too — one registry table (`SET_O_OPTION_NAMES`) now backs all five surfaces, killing a hardcoded parallel list.
- `SHELLOPTS` and `BASHOPTS` are readonly computed specials: colon-joined sorted enabled options, recomputed per read (dynamic — `set -e` adds errexit immediately), refusing assignment/unset like bash while execution continues, with `declare -p` attributes (`-r`/`-rx`) exact. Exported entries track option changes via a new `ShellOptions.on_change` observer, wired through fork cloning so a subshell's SHELLOPTS tracks its own changes independently of the parent.
- Inherited `SHELLOPTS`/`BASHOPTS` in the environment activate their options at startup (before anything runs — `SHELLOPTS=xtrace psh -c` traces the body), with bash's exported-only-if-inherited rule, vi/emacs coupling, IGNOREEOF binding, unknown-name warn (SHELLOPTS) vs silent (BASHOPTS) split, and empty-segment warnings via a faithful `extract_colon_unit` port (`SHELLOPTS=":"` warns exactly once).
- Cross-release interaction found by the verifier and fixed in-wave: the same-line brace-expansion scanner (v0.672) now recognizes `shopt -so/-uo braceexpand` toggles with exactly the `set` scanner's conservatism (pipeline/background discard, `( )` scoping, quoted/dynamic operands uninterpreted — parity verified like-for-like).
- Independently verified end-to-end: 95-row dev truth table plus the verifier's adversarial grammar rows and 14 refusal/attribute rows, 5/5 mutation arms killed by specific pins, red-on-base and red-on-tip both independently recounted, 9-item ledger reproduced, and the merged tree (with v0.674.0) gate-verified with all interaction probes green. Suite grows to 14,397 gate-passed (+107), compare-bash phase 1,218 with 16 `shellopts_*`/`bashopts_*` golden cases (1,241 total).
